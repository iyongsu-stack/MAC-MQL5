...
#include <BSPV4/Initv3.mqh>
#include <BSPV4/Deinit.mqh>
#include <BSPV4/IndicatorV3.mqh>
#include <BSPV4/ReadyCheckV3.mqh>
#include <BSPV4/OpenCloseV3.mqh>
   ...
   if( isNewBar(_Symbol) ) 
     {
      ...
      if(CloseAllTrading)
        {
         Alert("Heck");
         if(PositionSummary.totalNumPositions>0) CloseAllPosition();
        } 
      ...
      if( t1_PositionMode!=NoMode) MyOpenPosition(t1_PositionMode);  
                                               
      position_Mode t2_PositionMode = CloseSignal();      
      if( t2_PositionMode!=NoMode ) MyClosePosition(t2_PositionMode);

      if( CurBar==ReOC.NoModeBar)
        {
         if(StartTrading) OpenReadyCheck();
