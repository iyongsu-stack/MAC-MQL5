...
//TODO change ts prefix for trailing stop( see TrendStrength)
input   string              tsSettings          = " Trailing stop settings: ";   
input   double              tsValue             = 80.0;                 // Trailing Stop in pips
...
int OnInit()
{
    ...
    trailingStop    = PipToPoint(tsValue) * _Point;
...
//---
/* FOR TESTING PURPOSE
    Print("Trade transaction event...");
    if (trans.type == TRADE_TRANSACTION_DEAL_ADD) {

        if (HistoryDealGetInteger(trans.deal, DEAL_ENTRY) == DEAL_ENTRY_IN)             Print("DEAL_ENTRY_IN");

        else if (HistoryDealGetInteger(trans.deal, DEAL_ENTRY) == DEAL_ENTRY_OUT)       Print("DEAL_ENTRY_OUT");

        else if (HistoryDealGetInteger(trans.deal, DEAL_ENTRY) == DEAL_ENTRY_INOUT)     Print("DEAL_ENTRY_INOUT");
    }
*/
}   
//+------------------------------------------------------------------+
...
//| Like iTime for MT4                                               |
//+------------------------------------------------------------------+
datetime iTime1(string __symbol, ENUM_TIMEFRAMES __timeframe, int __shift)
{
