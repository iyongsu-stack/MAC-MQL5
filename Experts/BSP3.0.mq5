   ...
   if( BuyPositionInfo.numberOfPositions >= 1 )
     {
       ...
       if( (ShortWmaValue < BuyCloseReady.ShortStopLossBSP) &&  (askPrice < BuyPositionInfo.stopLossPrice) )
         {
          return(true);
         }

       if((ShortWmaValue > BuyCloseReady.ShortTakeProfitBSP) &&  (BuyCloseReady.OpenMode == SameDirection) )  
         {
   ...
   if( SellPositionInfo.numberOfPositions >= 1)   
     {
      ...
      if( (ShortWmaValue > SellCloseReady.ShortStopLossBSP) && (bidPrice > SellPositionInfo.stopLossPrice)) 
        {
         return(true);
        }
