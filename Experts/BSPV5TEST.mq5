...
input double              NoiseUpMultiFactor=2.0;   //
input double              NoiseDownMultiFactor=1.0;   //
...
input const int           BaseMagicNumber      = 200000;       // Magic Number
...
int   LASSTrendN, LASMTrendN, LASLTrendN, TrendSN, TrendMN, TrendLN,
      CurBar, CurSession,
      BasePositionMN[], CurrPositionMN[];                

const int MN_IncNumber=100, SessionIncNumber=5000, RORBars=10, TotalSession=10, Shift= 1;
...
int OnInit()
{
   FindSession.IsOpened = false;
   FindSession.ReadySession = 0;
   ...
   if( isNewBar(_Symbol) ) 
     {
      ...
      if(CloseAllTrading)
        {
         Alert("Heck");
         if(PositionSummary.totalNumPositions>0) { for(int i=m_Session;i<TotalSession;m_Session++){ ClosePositionBySession(session m_Session);} }
        } 

      if(StartTrading)
        {
         if(SessionMan.CandGo ) OpenReadyCheck(SessionMan.ReadySession);
        } 
      
      for(int m_Session=0;m_Session<TotalSession;i++)
        {
         if(CurPM[m_Session] != NoMode)
           {
            PositionModeCheck(m_Session);
      
            position_Mode t1_PositionMode = OpenSignal(); 
            if( t1_PositionMode!=NoMode) MyOpenPosition(m_Session);  
                                                     
            position_Mode t2_PositionMode = CloseSignal();      
            if( t2_PositionMode!=NoMode ) MyClosePosition(m_Session);
           } 
        } 
