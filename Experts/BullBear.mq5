...
input int                  EMAPeriod              = 50 ;       //Long Period
input int                  BBDIAvgPeriod          = 30;        //BBDI Avg Period
input double               BBDIMultiFactor        = 7.;        //BBDI Multi Factor

input double               LongShortThreshold     =-60.;       //Long Short Threshold
input double               NewPositionOpenVolume  = 0.003;     //New Position Open Volume
input int                  NewPositionOpenPeriod  = 3;         //New Position Open Period
input double               OpPositionCloseVolume  = 0.003;     //Open Position Close Volume
input int                  OpPositionClosePeriod  = 3;         //Open Position Close Period
...
struct curState
{
   bool MainPositionOpened=false;
   bool TempPositionOpened=false;
};
curState CurrentStatus;
...
struct poInfo
{
   ...
   double SizeOfFirstPosition=0.;
   double SizeOfTotalPositions=0.;
   double TotalProfit=0.;
};
poInfo MainPositionInfo, TempPositionInfo;
...
Trend LongTrend, LongTempTrend, ShortTrend, ShortTempTrend;


int LongHandle=INVALID_HANDLE;
int ShortHandle = INVALID_HANDLE;
int LongBBDIHandle = INVALID_HANDLE;
int ShortBBDIHandle = INVALID_HANDLE;
int BBTAHandle = INVALID_HANDLE;

double LongBBIBuffer[],LongBBICBuffer[], ShortBBIBuffer[], ShortBBICBuffer[],
       LongBBDINonBuffer[], LongBBDIBuffer[], ShortBBDINonBuffer[], ShortBBDIBuffer[], BBTABuffer[];
...
int OnInit(){
   ...
   // Loading indicators..
   LongHandle=iCustom(_Symbol,_Period,IND1, LongPeriod);
   if(LongHandle==INVALID_HANDLE){
      Alert("Error when loading the Long BBI indicator");
      return(-1);
   }  

   ShortHandle=iCustom(_Symbol,_Period,IND1, ShortPeriod);
   if(ShortHandle==INVALID_HANDLE){
      Alert("Error when loading the Short BBI indicator");
      return(-1);
   }  

   LongBBDIHandle=iCustom(_Symbol,_Period,IND2, LongPeriod);
   if(BBDIHandle==INVALID_HANDLE){
      Alert("Error when loading the Long BBDI indicator");
      return(-1);
   }  
   
   ShortBBDIHandle=iCustom(_Symbol,_Period,IND2, ShortPeriod);
   if(BBDIHandle==INVALID_HANDLE){
      Alert("Error when loading the Long BBDI indicator");
      return(-1);
   }  
   ...
   BBTAHandle=iCustom(_Symbol,_Period,IND3, BBTAPeriod);
   if(BBTAHandle==INVALID_HANDLE){
      Alert("Error when loading the the BBTA indicator");
      return(-1);
   }  
   ...
   ArraySetAsSeries(LongBBIBuffer, true);
   ArraySetAsSeries(ShortBBIBuffer, true);
   ArraySetAsSeries(BBDIBuffer, true);
   ArraySetAsSeries(BBTABuffer, true);
...
void OnTick(){
   ...
   bool longCloseBuy=false,
        longCloseSell=false,
        longOpenBuy=false,
        longOpenSell=false,
        shortCloseBuy=false,
        shortCloseSell=false,
        shortOpenBuy=false,
        shortOpenSell=false;
...
bool Indicators(){
   if(
      CopyBuffer(LongHandle,0,Shift,1, LongBBIBuffer)==-1             ||
      CopyBuffer(LongHandle,1,Shift,2, LongBBICBuffer)==-1            ||
      CopyBuffer(ShortHandle,0,Shift,1, ShortBBIBuffer)==-1           ||
      CopyBuffer(ShortHandle,1,Shift,2, ShortBBICBuffer)==-1          ||
      CopyBuffer(LongBBDIHandle,0,Shift,1,LongBBDINonBuffer)==-1      ||
      CopyBuffer(LongBBDIHandle,2,Shift,1,LongBBIBuffer)==-1          ||
      CopyBuffer(ShortBBDIHandle,0,Shift,1,ShortBBDINonBuffer)==-1    ||
      CopyBuffer(ShortBBDIHandle,2,Shift,1,ShortBBDIBuffer)==-1       ||
      CopyBuffer(BBTAHandle,0,Shift,1,BBTABuffer)==-1  
   ){
      return(false);
   }
