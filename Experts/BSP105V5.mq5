...
#include <BSPV5/InitV5.mqh>
#include <BSPV5/DeinitV5.mqh>
#include <BSPV5/IndicatorV5.mqh>
   ...
   if( isNewBar(_Symbol) ) 
     {
      ...
      if(StartTrading && SessionMan.CanGo)
        {
      ...
      for(int m_Session=0;m_Session<TotalSession;m_Session++)
        {
         if(CurPM[m_Session] != NoMode)
           {
            if(PositionSummary[m_Session].totalNumPositions >=1) 
              {
               PositionModeCheck(m_Session);
              } 
      
            position_Mode t1_PositionMode = OpenSignal(m_Session); 
            if( t1_PositionMode!=NoAction) MyOpenPosition(m_Session);  
                                                     
            position_Mode t2_PositionMode = CloseSignal(m_Session);      
            if( t2_PositionMode!=NoAction ) MyClosePosition(m_Session);
