...
input int                  InpShortNLRPeriod   = 20;           //NLR-MA Short Period
input int                  InpLongNLRPeriod    = 30;           //NLR-MA Short Period
...
trend NLRShortTrend, NLRLongTrend;
...
int BSPHandle=INVALID_HANDLE,
    ...
    NLRShortHandle = INVALID_HANDLE,
    NLRLongHandle = INVALID_HANDLE;

double BSPUpBand[], BSPDownBand[], BSPBuffer[], BSPLRBuffer[], NLRLongBuffer[], NLRShortBuffer[];
...
int OnInit(){
   ...
   NLRShortHandle      = iCustom(_Symbol,PERIOD_M1,IND3, InpShortNLRPeriod);
   NLRLongHandle       = iCustom(_Symbol,PERIOD_M1,IND3, InpLongNLRPeriod);


   if(BSPHandle==INVALID_HANDLE || BSPLRHandle==INVALID_HANDLE || 
      NLRShortHandle==INVALID_HANDLE || NLRLongHandle==INVALID_HANDLE){
   ...
   ArraySetAsSeries(NLRLongBuffer, true);
   ArraySetAsSeries(NLRShortBuffer, true);
...
void OnDeinit(const int reason){
   ...
   if(NLRShortHandle!=INVALID_HANDLE) IndicatorRelease(NLRShortHandle);
   if(NLRLongHandle!=INVALID_HANDLE) IndicatorRelease(NLRLongHandle);   
}
...
void OnTick(){
   ...
   if(Times())
   {
      ...
      if(OpenBuy ) OpenPosition(POSITION_TYPE_BUY);
      if(OpenSell) OpenPosition(POSITION_TYPE_SELL);
...
bool SignalOpenBuy(){
   ...
   if( (LongPositionInfo.numberOfPositions==0) && (BSPState == Lower) && 
       (BSPLRState == Plus) && (NLRShortTrend ==UpTrend) ) return(true);
   else return(false);
...
bool SignalOpenSell(){
   ...
   if( (ShortPositionInfo.numberOfPositions==0) && (BSPState == Upper) && 
       (BSPLRState == Minus) && (NLRShortTrend ==DownTrend) ) return(true);
   else return(false);
...
bool SignalCloseBuy(){
   ...
   if( (LongPositionInfo.numberOfPositions>=1) && (BSPState != Lower) && 
       (NLRShortTrend == DownTrend)) return(true);
   return(false);
}
...
bool SignalCloseSell(){
   ...
   if( (ShortPositionInfo.numberOfPositions>=1) && (BSPState != Upper) && 
       (NLRShortTrend == UpTrend)) return(true);
   return(false);
...
bool Indicators(){
   ...
   if(CopyBuffer(BSPHandle,0,Shift,1,BSPBuffer)==-1              ||
      CopyBuffer(BSPHandle,1,Shift,1,BSPUpBand)==-1              ||
      CopyBuffer(BSPHandle,2,Shift,1,BSPDownBand)==-1            ||
      CopyBuffer(BSPLRHandle,0,Shift,1,BSPLRBuffer)==-1          ||
      CopyBuffer(NLRShortHandle,1,Shift,1,NLRShortBuffer)==-1    ||
      CopyBuffer(NLRLongHandle,1,Shift,1,NLRLongBuffer)==-1 ){
      return(false);
   }
   ...
   if( (int)NormalizeDouble(NLRShortBuffer[0], 0) == 2) NLRShortTrend = UpTrend;
   else if( (int)NormalizeDouble(NLRShortBuffer[0], 0) == 1) NLRShortTrend = DownTrend;
   else NLRShortTrend = NoTrend; 
   
   if( (int)NormalizeDouble(NLRLongBuffer[0],0) == 2) NLRLongTrend = UpTrend;
   else if((int)NormalizeDouble(NLRLongBuffer[0], 0) == 1) NLRLongTrend = DownTrend;
   else NLRLongTrend = NoTrend; 
