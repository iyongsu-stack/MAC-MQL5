...
input int                 WmaPeriodS           = 8;          // wmaPeriodS
       ...
       if( (BuyCloseReady.OpenMode == SameDirection) )  
         {
          if((ShortWmaValue > BuyCloseReady.ShortTakeProfitBSP) && BuyCloseReady.ShortWmaReady &&
              BuyCloseReady.NonLReady && BuyCloseReady.ASTReady)
            { 
             return(true);
            }
         }
       ...
       if( (SellCloseReady.OpenMode == SameDirection) )  
         {
          if((ShortWmaValue <= SellCloseReady.ShortTakeProfitBSP) && SellCloseReady.ShortWmaReady &&
              SellCloseReady.NonLReady && SellCloseReady.ASTReady)    
            { 
             return(true);
            }
         }
...
// Same Direction Sell Check       
      ...
      else if( BuyOpenReady.ASReady && (ASTTrend == DownTrend) &&
...
// Same Direction Buy Check
      ...
      else if( SellOpenReady.ASReady && (ASTTrend == UpTrend) &&
