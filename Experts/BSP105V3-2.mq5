...
input int                 NonLRPeriod          = 20;          // NonLRPeriodS
...
sinput   const string  Message6="";// AvgStd2
input int                 ASTLwmaPeriod         = 3;           // ASTWmaPeriod1
input int                 ASTAvgPeriod          = 10;          // ASTAvgPeriod
input int                 ASTStdPeriodL         = 10000;       // ASTStdPeriodL
input int                 ASTStdPeriodS         = 7;          // ASTStdPeriodS
input double              ASTMultiFactorL1      = 0.5;         // ASTStdMultiFactorL1
input double              ASTMultiFactorL2      = 1.5;         // ASTStdMultiFactorL2
input double              ASTMultiFactorL3      = 2.5;         // ASTStdMultiFactorL3
...
BandState ASBand, ASTBand, WmaBand;
...
trend ShortWmaTrend, LongWmaTrend, LLongWmaTrend, ASTrend, ASTTrend, NonLRTrend,
      ShortWmaTrend1, LongWmaTrend1, LLongWmaTrend1, ASTrend1, ASTrend2, ASTTrend1, ASTTrend2, 
      NonLRTrend1, NonLRTrend2;
...
double NonLRBuffer[], NonLRColorBuffer[],
       ...
       ASTBuffer[], ASTColorBuffer[], 
       ASTP1Band[], ASTP2Band[], ASTP3Band[], ASTM1Band[], ASTM2Band[], ASTM3Band[],
...
int OnInit()
{
   ...
   // Loading indicators..
   NonLRHandle       = iCustom(_Symbol,_Period, IND1, NonLRPeriod );
   WmaHandleS        = iCustom(_Symbol,_Period, IND7, WmaPeriodS, WmaPeriod2, WmaStdPeriod, WmaMultiFactor1, 
                                                       WmaMultiFactor2, WmaMultiFactor3 );
   WmaHandleL        = iCustom(_Symbol,_Period, IND3, WmaPeriodL, WmaPeriod2);
   WmaHandleLL       = iCustom(_Symbol,_Period, IND3, WmaPeriodLL, WmaPeriod2);   
   ASHandle          = iCustom(_Symbol,_Period, IND4, ASLwmaPeriod, ASAvgPeriod, ASStdPeriodL, ASStdPeriodS, 
                                                       ASMultiFactorL1, ASMultiFactorL2, ASMultiFactorL3);
   ASTHandle         = iCustom(_Symbol,_Period, IND4, ASTLwmaPeriod, ASTAvgPeriod, ASTStdPeriodL, ASTStdPeriodS, 
                                                       ASTMultiFactorL1, ASTMultiFactorL2, ASTMultiFactorL3);
   BSPHandle         = iCustom(_Symbol,_Period, IND6, BSPWmaPeriod, BSPMultiRatio );  
   HLHandle          = iCustom(_Symbol,_Period, IND8, HLWmaPeriod, HLMultiRatio );  


   if( NonLRHandle     == INVALID_HANDLE  || WmaHandleS      == INVALID_HANDLE || WmaHandleL      == INVALID_HANDLE || 
       WmaHandleLL     == INVALID_HANDLE  || ASHandle        == INVALID_HANDLE || ASTHandle       == INVALID_HANDLE ||
       BSPHandle       == INVALID_HANDLE  || HLHandle        == INVALID_HANDLE )
   {
   ...
   ArraySetAsSeries(ASTBuffer, true);
   ArraySetAsSeries(ASTColorBuffer, true);
   ArraySetAsSeries(ASTP1Band, true);
   ArraySetAsSeries(ASTP2Band, true);
   ArraySetAsSeries(ASTP3Band, true);
   ArraySetAsSeries(ASTM1Band, true);
   ArraySetAsSeries(ASTM2Band, true);
   ArraySetAsSeries(ASTM3Band, true);
...
void OnDeinit(const int reason)
{
   ...
   if(ASTHandle       !=  INVALID_HANDLE)  IndicatorRelease(ASHandle);  
...
//          return(true);
       ...
       if( (BuyCloseReady.OpenMode == OppositeDirection) )  
         {
          if(BuyCloseReady.ShortWmaReady && BuyCloseReady.NonLReady && BuyCloseReady.ASReady)
            { 
             return(true);
            }
         }
...
//         return(true);
       ...
       if( (SellCloseReady.OpenMode == OppositeDirection) )  
         {
          if( SellCloseReady.ShortWmaReady && SellCloseReady.NonLReady && SellCloseReady.ASReady)    
            { 
             return(true);
            }
         }
   ...
   if( BuyPositionInfo.numberOfPositions >= 1 )
     {
      ...
      if( (NonLRTrend == DownTrend)) BuyCloseReady.NonLReady = true; 
      else BuyCloseReady.NonLReady = false;      
      
      if( (ASTrend == DownTrend)) BuyCloseReady.ASReady = true; 
      else BuyCloseReady.ASReady = false;      

      if( (ASTTrend == DownTrend)) BuyCloseReady.ASTReady = true; 
      else BuyCloseReady.ASTReady = false;      
   ...
   if( SellPositionInfo.numberOfPositions >= 1)   
     {
      ...
      if((NonLRTrend == UpTrend))  SellCloseReady.NonLReady = true;
      else SellCloseReady.NonLReady = false;

      if( (ASTrend == UpTrend)) SellCloseReady.ASReady = true; 
      else SellCloseReady.ASReady = false;      

      if( (ASTTrend == UpTrend)) SellCloseReady.ASTReady = true; 
      else SellCloseReady.ASTReady = false;      
...
// Oppossite Direction
   if( (TotalNumberOfPositions <= 0) && BuyOpenReady.ShortWmaReady )
     {
...
// Opposite Direction
   if( (TotalNumberOfPositions <= 0) && SellOpenReady.ShortWmaReady )
     {
...
//  Opposite Direction Buy Open Ready Check
      if( (ASBand == BandM2 || ASBand == BandM3) && (ASTrend == UpTrend)  ) 
        {
      ...
      if( BuyOpenReady.ASReady && (ShortWmaTrend == UpTrend) ) 
        {
...
//  Opposite Direction Sell Ready Check
      if( (ASBand == BandP2 || ASBand == BandP3) && (ASTrend == DownTrend) )
        {
      ...
      if( SellOpenReady.ASReady && (ShortWmaTrend == DownTrend)) 
        {
...
bool Indicators()
{
   ...
   if(CopyBuffer(NonLRHandle,      0,  Shift,  1, NonLRBuffer)            == -1   ||
      ...
      CopyBuffer(ASTHandle,        0,  Shift,  1, ASTP3Band)              == -1   ||
      CopyBuffer(ASTHandle,        1,  Shift,  1, ASTP2Band)              == -1   ||
      CopyBuffer(ASTHandle,        2,  Shift,  1, ASTP1Band)              == -1   ||
      CopyBuffer(ASTHandle,        3,  Shift,  1, ASTM1Band)              == -1   ||
      CopyBuffer(ASTHandle,        4,  Shift,  1, ASTM2Band)              == -1   ||
      CopyBuffer(ASTHandle,        5,  Shift,  1, ASTM3Band)              == -1   ||
      CopyBuffer(ASTHandle,        6,  Shift,  2, ASTBuffer)              == -1   ||
      CopyBuffer(ASTHandle,        7,  Shift,  1, ASTColorBuffer)         == -1   ||
   ...
   if      (ASTBuffer[0] > ASP3Band[0])         ASTBand = BandP3;
   else if (ASTBuffer[0] > ASP2Band[0])         ASTBand = BandP2;
   else if (ASTBuffer[0] > ASP1Band[0])         ASTBand = BandP1; 
   else if (ASTBuffer[0] > 0. )                 ASTBand = BandP0;
   else if (ASTBuffer[0] > ASM1Band[0])         ASTBand = BandM0;
   else if (ASTBuffer[0] > ASM2Band[0])         ASTBand = BandM1;
   else if (ASTBuffer[0] > ASM3Band[0])         ASTBand = BandM2;
   else                                         ASTBand = BandM3; 
   
   ASTTrend2 = ASTTrend1;
   ASTTrend1 = ASTTrend;
   if( (int)NormalizeDouble(ASTColorBuffer[0], 0) == 1) ASTTrend = DownTrend;
   else ASTTrend = UpTrend;   
...
void CloseReadyReset(void)
{
   ...
   if( BuyPositionInfo.numberOfPositions < 1 )
     {
      BuyCloseReady.NonLReady = false;
      BuyCloseReady.ShortWmaReady = false;
      BuyCloseReady.LongWmaReady = false;
      BuyCloseReady.ASReady = false;
      BuyCloseReady.ASTReady = false;
   ...
   if( SellPositionInfo.numberOfPositions < 1)
     {
      SellCloseReady.NonLReady = false;
      SellCloseReady.ShortWmaReady = false;
      SellCloseReady.LongWmaReady = false;
      SellCloseReady.ASReady = false;
      SellCloseReady.ASTReady = false;
