...
3. **절대값 사용 엄격 금지 (파생 피처 사용)**
   - 원본 절대값 그대로 투입 금지. 기본 5유형(변화율, Z-Score, 비율, Slope, **롤링 퍼센타일 랭크 pct240/pct1440**)으로 변환 후 사용.

### 부가 원칙
- **🎯 확정된 전략 구조 (2026-03-09 갱신)**: AI A+B+C 통합 모델 → TrailingStop 청산
  ```
  AI:      80개 피처(SHAP Top-60+pct16+regime4) + 단조제약 + FP페널티
...
- **메가 피처 풀**: ~817개 투입 → Spearman 0.85 Pruning(419개) → SHAP Top-60 자동 선별.
