...
void OnInit()
  {
   ...
   string short_name = "BSPLRAVGSTD_REL("+ (string)LwmaPeriod + ", "  + (string)AvgPeriod + ", " +
   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       if(tempTotalPressure==0) 
        { tempBuyRatio=0.; tempSellRatio=0.; }
       else
        {
       ...
       tempDiffPressure = (MathAbs(tempBuyRatio) - MathAbs(tempSellRatio));
