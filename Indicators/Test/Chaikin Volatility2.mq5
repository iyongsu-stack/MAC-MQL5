...
void OnInit() {
  ...
  if (InpSmoothPeriod <= 0) {
    ExtSmoothPeriod = 30;
    printf("Incorrect value for input variable InpSmoothPeriod=%d. Indicator "
           "will use value=%d for calculations.",
  ...
  if (InpCHVPeriod <= 0) {
    ExtCHVPeriod = 30;
    printf("Incorrect value for input variable InpCHVPeriod=%d. Indicator will "
           "use value=%d for calculations.",
  ...
  if (InpStdDevPeriod <= 0) {
    ...
    printf("Incorrect value for input variable InpStdDevPeriod=%d. Indicator "
           "will use value=%d for calculations.",
  ...
  if (InpMultiFactor1 <= 0.0)
    printf("WARNING: Incorrect value for input variable InpMultiFactor1=%.2f. "
           "Should be positive.",
           InpMultiFactor1);
  if (InpMultiFactor2 <= 0.0)
    printf("WARNING: Incorrect value for input variable InpMultiFactor2=%.2f. "
           "Should be positive.",
           InpMultiFactor2);
  if (InpMultiFactor3 <= 0.0)
    printf("WARNING: Incorrect value for input variable InpMultiFactor3=%.2f. "
           "Should be positive.",
           InpMultiFactor3);
  ...
  IndicatorSetString(
      INDICATOR_SHORTNAME,
      "Chaikin Volatility(" + string(ExtSmoothPeriod) + "," +
          string(ExtCHVPeriod) + "," + MAName + "," + string(ExtStdDevPeriod) +
          "," + string(InpMultiFactor1) + "," + string(InpMultiFactor2) + "," +
          string(InpMultiFactor3) + ")");
  //--- round settings
  ...
  if (CheckPointer(iStdDev3) == POINTER_INVALID) {
    Print("ERROR: HiStdDev3 객체 생성 실패! 인디케이터가 제대로 작동하지 않을 "
          "수 있습니다.");
    // 객체 생성 실패 시 초기화 중단하지 않고 계속 진행하되, OnCalculate에서
    // null 체크 필요
  }
  //---- OnInit done
}
...
int OnCalculate(const int rates_total, const int prev_calculated,
  ...
  if (CheckPointer(iStdDev3) == POINTER_INVALID) {
    ...
    Print("WARNING: HiStdDev3 객체가 유효하지 않습니다. StdDev 밴드가 계산되지 "
          "않습니다.");
  }
  ...
  if (prev_calculated < 1 || prev_calculated > rates_total) {
    ...
    // iStdDev3 객체가 내부적으로 이전 계산 상태(누적값 등)를 가지고 있을 수
    // 있으므로 전체 재계산 시 객체를 새로 생성하여 상태를 리셋해야 합니다.
    if (CheckPointer(iStdDev3) == POINTER_DYNAMIC)
      delete iStdDev3;
  ...
  if (InpSmoothType == SMA)
    SimpleMAOnBuffer(rates_total, prev_calculated, 0, ExtSmoothPeriod,
                     ExtHLBuffer, ExtSHLBuffer);
  else if (InpSmoothType == EMA)
    ExponentialMAOnBuffer(rates_total, prev_calculated, 0, ExtSmoothPeriod,
                          ExtHLBuffer, ExtSHLBuffer);
  else if (InpSmoothType == WMA)
    LinearWeightedMAOnBuffer(rates_total, prev_calculated, 0, ExtSmoothPeriod,
                             ExtHLBuffer, ExtSHLBuffer);
  ...
  for (i = pos; i < rates_total && !IsStopped(); i++) {
    ...
    if (ExtSHLBuffer[prevIdx] != 0.0 && MathAbs(ExtSHLBuffer[prevIdx]) > 1e-10)
      ExtCHVBuffer[i] = 100.0 * (ExtSHLBuffer[i] - ExtSHLBuffer[prevIdx]) /
                        ExtSHLBuffer[prevIdx];
    else
      ExtCHVBuffer[i] = 0.0;

    //--- set CHV color (0=Green for rising, 1=Red for falling)
    if (i > 0 && ExtCHVBuffer[i] != EMPTY_VALUE &&
        ExtCHVBuffer[i - 1] != EMPTY_VALUE) {
