   ...
   if(rates_total > 0 && !g_IsWritten) 
     {
      ...
      if(handle != INVALID_HANDLE) 
        {
         FileWrite(handle, "Time", "Open", "Close", "High", "Low", "TickVolume", "BWMFI", "BWMFI_Color");
         
         for(int k=0; k<rates_total; k++) 
           {
            string timeStr = TimeToString(time[k], TIME_DATE|TIME_MINUTES);
            // Write requested values: Time, OCHL, TickVolume, ExtMFIBuffer, ExtColorBuffer
            FileWrite(handle, timeStr, open[k], close[k], high[k], low[k], tick_volume[k], ExtMFIBuffer[k], ExtColorBuffer[k]);
           }
         FileClose(handle);
