...
//--- Расчёт индикатора
   ...
   if(rates_total > 0 && !g_IsWritten)
     {
      string filename = "raw\\TradesDynamicIndex_DownLoad.csv";
      int handle = FileOpen(filename, FILE_CSV|FILE_WRITE|FILE_ANSI);

      if(handle != INVALID_HANDLE)
        {
         FileWrite(handle, "Time", "Open", "Close", "High", "Low", "TrSi", "Signal");

         for(int k=0; k<rates_total; k++)
           {
            // Note: Buffers are AsSeries=true (0=Newest), but time[] is usually Oldest->Newest.
            // We must reverse the buffer index to align with time[k].
            // Time[0] (Oldest) matches Buffer[rates_total-1] (Oldest).
            string timeStr = TimeToString(time[k], TIME_DATE|TIME_MINUTES);
            
            // Adjust index for Series buffers
            int seriesIndex = rates_total - 1 - k;
            
            FileWrite(handle, timeStr, open[k], close[k], high[k], low[k], BufferTRSI[seriesIndex], BufferSignal[seriesIndex]);
           }
         FileClose(handle);
