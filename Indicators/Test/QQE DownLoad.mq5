...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   for(int i = start; i < rates_total; i++)
     {
      ...
      // Initialize if start of data or previous was invalid
   ...
   if(rates_total > 0 && !g_IsWritten) {
      string filename = "QQE_DownLoad.csv";
      int handle = FileOpen(filename, FILE_CSV|FILE_WRITE|FILE_ANSI);
      
      if(handle != INVALID_HANDLE) {
         FileWrite(handle, "Time", "Open", "Close", "High", "Low", "RsiMa", "TrLevelSlow");
         
         for(int k=0; k<rates_total; k++) {
            string timeStr = TimeToString(time[k], TIME_DATE|TIME_MINUTES);
            FileWrite(handle, timeStr, open[k], close[k], high[k], low[k], RsiMaBuffer[k], TrLevelSlowBuffer[k]);
         }
         FileClose(handle);
