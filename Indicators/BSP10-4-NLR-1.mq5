...
input int                 WmaPeriod     = 30;          // WmaPeriod1
input int                 LRPeriod      = 10;           // LRPeriod
input int                 AvgPeriod     = 120;          // AvgPeriod
input int                 StdPeriod     = 2880;        // StdPeriod
input double              MultiFactor1  = 1.5;         // StdMultiFactor1
input double              MultiFactor2  = 2.5;         // StdMultiFactor2
...
double SellPressure[], BuyPressure[],
       ...
       up2StdAvgValLR[], down2StdAvgValLR[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(6,stdAvgValLR,INDICATOR_CALCULATIONS);
   SetIndexBuffer(7,val,INDICATOR_CALCULATIONS);
...
*/
       ...
       BuyPressure[bar] = (tempBuyRatio+BuyPressure[bar-1])/2.;
       SellPressure[bar] = (tempSellRatio+SellPressure[bar-1])/2.; 
     }
...
*/
  ...
  for(int bar=seventh; bar<rates_total; bar++)
     {        
       ...
       upStdAvgValLR[bar] = avgValLR[bar] + standardDeviation * MultiFactor1;
       downStdAvgValLR[bar] = avgValLR[bar] - standardDeviation * MultiFactor1;
       up2StdAvgValLR[bar] = avgValLR[bar] + standardDeviation * MultiFactor2;
       down2StdAvgValLR[bar] = avgValLR[bar] - standardDeviation * MultiFactor2;
