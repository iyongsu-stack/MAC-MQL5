   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       if(bar>=fourth)
       {
         double Std = StdDev3(bar, StdPeriodL, BuyPressure, SellPressure);
         if( BuyPressure[bar] > (Std*MaxBSPMult) ) BuyPressure[bar] = Std*MaxBSPMult;
         if( SellPressure[bar] > (Std*MaxBSPMult) ) SellPressure[bar] = Std*MaxBSPMult;
       }              
...
double StdDev3(int end, int SDPeriod, const double &S1_Array[], const double &S2_Array[])
{
    double dAmount=0., StdValue=0.;  

    for(int i=end+1-SDPeriod;i<=end;i++)
    {
      dAmount+=(S1_Array[i])*(S1_Array[i]);
      dAmount+=(S2_Array[i])*(S2_Array[i]);
    }       

    StdValue = MathSqrt(dAmount/(SDPeriod*2));
    return(StdValue);
}  
