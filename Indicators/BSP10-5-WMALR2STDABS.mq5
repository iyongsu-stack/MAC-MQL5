...
input double              MultiFactorL1  = 1.3;         // StdMultiFactorL1
input double              MultiFactorL2  = 2.0;         // StdMultiFactorL2
input double              MultiFactorL3  = 1.3;         // StdMultiFactorL3

input double              MultiFactorS1  = 1.3;         // StdMultiFactorS1
input double              MultiFactorS2  = 2.0;         // StdMultiFactorS2
input double              MultiFactorS3  = 1.3;         // StdMultiFactorS3
...
void OnInit()
  {
   ...
   SetIndexBuffer(4,up2StdAvgValLR,INDICATOR_DATA); 
   SetIndexBuffer(5,down2StdAvgValLR,INDICATOR_DATA);  
   SetIndexBuffer(6,up3StdAvgValLR,INDICATOR_DATA); 
   SetIndexBuffer(7,down3StdAvgValLR,INDICATOR_DATA);  
   ...
   string short_name = "BSPLR2STD("+ (string)LwmaPeriod + ", " + (string)LRPeriod + ", " + 
                                            (string)StdPeriodL + ", " + (string)StdPeriodS + ",, " + 
                                            (string)MultiFactorL1 + ", " + (string)MultiFactorL2 + ", " + 
                                            (string)MultiFactorL3+ ",,  " + (string)MultiFactorS1 + ", " + 
                                            (string)MultiFactorS2+ ", " + (string)MultiFactorS3 +   ")";      
...
*/       
  ...
  for(int bar=third; bar<rates_total; bar++)
     {        
       ...
       standardDeviationS = StdDev(bar, StdPeriodS, valLR);

       if( (standardDeviationL*MultiFactorL1) >= (standardDeviationS*MultiFactorS1) )
         {
         ...
         {
            up1StdAvgValLR[bar] = standardDeviationS * MultiFactorS1;
            down1StdAvgValLR[bar] = (-1) * standardDeviationS * MultiFactorS1;
            
            up2StdAvgValLR[bar] = standardDeviationS * MultiFactorS2;
            down2StdAvgValLR[bar] = (-1) * standardDeviationS * MultiFactorS2;

            up3StdAvgValLR[bar] = standardDeviationS * MultiFactorS3;
            down3StdAvgValLR[bar] = (-1) * standardDeviationS * MultiFactorS3;
