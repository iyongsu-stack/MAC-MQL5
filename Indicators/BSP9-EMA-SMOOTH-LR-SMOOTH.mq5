...
input uint                XLength2       = 2;         // LR Smooth Depth
input int                 XPhase2        = 5;         // LR Smoothing parameter
...
double BSPDiff[], LR_BSPDiff[], temp_LR_BSPDiff[],
...
int    starting_bar, starting_bar2;
...
void OnInit()
  {
   ...
   SetIndexBuffer(1,temp_LR_BSPDiff,INDICATOR_DATA);   
   SetIndexBuffer(2,BSPDiff,INDICATOR_CALCULATIONS);   
   SetIndexBuffer(3,dema_BuyPressure,INDICATOR_CALCULATIONS);   
   SetIndexBuffer(4,dema_SellPressure,INDICATOR_CALCULATIONS);  
   SetIndexBuffer(5,dema_TotalPressure,INDICATOR_CALCULATIONS);      
   SetIndexBuffer(6,ema_BuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(7,ema_SellPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(8,wma_TotalPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(9,avg_BuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(10,avg_SellPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(11,SellPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(12,BuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(13,TotalPressure,INDICATOR_CALCULATIONS);
  
   starting_bar = 2+AvgPeriod+EmaPeriod;
   starting_bar2 = 2+AvgPeriod+EmaPeriod+LRPeriod;
...
int OnCalculate(const int rates_total,    // number of bars in history at the current tick
                ...
                const int &spread[])
  {
   ...
   double mVolume, temp_BSPDiff, ema_BSPDiff, dema_BSPDiff, 
          ema_LR_BSPDiff, dema_LR_BSPDiff;
...
//---- The main loop of the indicator calculation
  ...
  for(int bar=fourth; bar<rates_total; bar++)
     {        
       temp_LR_BSPDiff[bar] = LinearRegression(bar,LRPeriod, BSPDiff);
       ema_LR_BSPDiff = XMA4.XMASeries(starting_bar2,prev_calculated,rates_total,(int)MODE_EMA,
                                    XPhase2,XLength2,temp_LR_BSPDiff[bar], bar,false);
       dema_LR_BSPDiff = XMA5.XMASeries(starting_bar2,prev_calculated,rates_total,(int)MODE_EMA,
                                    XPhase2,XLength2,ema_LR_BSPDiff, bar,false);
       double aaaa = XMA6.XMASeries(starting_bar2,prev_calculated,rates_total,(int)MODE_EMA,
                                    XPhase2,XLength2,dema_LR_BSPDiff, bar,false);
       LR_BSPDiff[bar] = aaaa;
