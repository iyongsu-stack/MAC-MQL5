...
input int      Periods =250;
input ENUM_MA_METHOD MA_METHOD = MODE_EMA;
input ENUM_APPLIED_PRICE inpPrice    = PRICE_CLOSE; // Price


//--- indicator buffers
double VWAPBuffer[], mtf_VWAPBuffer[];
double price[];
ENUM_TIMEFRAMES  tf;
int mtf_handle;
...
//--- number of digits of indicator value
   ...
   if(TimeFrame > 0) 
   {
      mtf_handle = iCustom(Symbol(),TimeFrame,"VWAPMTF",TimeFrame,Periods,MA_METHOD,inpPrice );
   }   
//---
                ...
                const int &spread[])
{
   ...
   int pos, mtflimit, shift, x, y, copied;
   datetime mtf_time;
   ...
   else 
   {
      ...
      pos=prev_calculated-1; // starting number for calculation of new bars
   ...
   if(tf > Period())
   {
      ...
      for(shift=0,y=0;shift<mtflimit;shift++)
      {
         ...
         copied = CopyBuffer(mtf_handle,0,mtf_time,mtf_time,mtf_VWAPBuffer);
         if(copied <= 0) return(0);
      
         VWAPBuffer[x] = mtf_VWAPBuffer[0];      
      }
   }
   else
   {
