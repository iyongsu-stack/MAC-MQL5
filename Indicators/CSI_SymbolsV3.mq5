...
//--- assignment of array to indicator buffer
   ...
   SetIndexBuffer(36,t1_AUDUSDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(37,t2_AUDUSDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(38,t1_EURGBPx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(39,t2_EURGBPx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(40,t1_EURJPYx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(41,t2_EURJPYx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(42,t1_EURUSDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(43,t2_EURUSDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(44,t1_GBPAUDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(45,t2_GBPAUDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(46,t1_GBPUSDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(47,t2_GBPUSDx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(48,t1_USDCADx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(49,t2_USDCADx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(50,t1_USDCHFx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(51,t2_USDCHFx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(52,t1_USDJPYx,INDICATOR_CALCULATIONS);
   SetIndexBuffer(53,t2_USDJPYx,INDICATOR_CALCULATIONS);
...
//--- set AsSeries
   ...
   ArraySetAsSeries(t1_AUDUSDx,true);
   ArraySetAsSeries(t2_AUDUSDx,true);
   ArraySetAsSeries(t1_EURGBPx,true);
   ArraySetAsSeries(t2_EURGBPx,true);
   ArraySetAsSeries(t1_EURJPYx,true);
   ArraySetAsSeries(t2_EURJPYx,true);
   ArraySetAsSeries(t1_EURUSDx,true);
   ArraySetAsSeries(t2_EURUSDx,true);
   ArraySetAsSeries(t1_GBPAUDx,true);
   ArraySetAsSeries(t2_GBPAUDx,true);
   ArraySetAsSeries(t1_GBPUSDx,true);
   ArraySetAsSeries(t2_GBPUSDx,true);
   ArraySetAsSeries(t1_USDCADx,true);
   ArraySetAsSeries(t2_USDCADx,true);
   ArraySetAsSeries(t1_USDCHFx,true);
   ArraySetAsSeries(t2_USDCHFx,true);
   ArraySetAsSeries(t1_USDJPYx,true);
   ArraySetAsSeries(t2_USDJPYx,true);

   ArrayInitialize(t1_AUDUSDx,0.);
   ArrayInitialize(t2_AUDUSDx,0.);
   ArrayInitialize(t1_EURGBPx,0.);
   ArrayInitialize(t2_EURGBPx,0.);
   ArrayInitialize(t1_EURJPYx,0.);
   ArrayInitialize(t2_EURJPYx,0.);
   ArrayInitialize(t1_EURUSDx,0.);
   ArrayInitialize(t2_EURUSDx,0.);
   ArrayInitialize(t1_GBPAUDx,0.);
   ArrayInitialize(t2_GBPAUDx,0.);
   ArrayInitialize(t1_GBPUSDx,0.);
   ArrayInitialize(t2_GBPUSDx,0.);
   ArrayInitialize(t1_USDCADx,0.);
   ArrayInitialize(t2_USDCADx,0.);
   ArrayInitialize(t1_USDCHFx,0.);
   ArrayInitialize(t2_USDCHFx,0.);
   ArrayInitialize(t1_USDJPYx,0.);
   ArrayInitialize(t2_USDJPYx,0.);
...
// relative strength of symbols
         ...
         EURCADx[i]=EUR-CAD;
         EURCHFx[i]=EUR-CHF;
         t1_EURGBPx[i]=EUR-GBP;
         t1_EURJPYx[i]=EUR-JPY;
         EURNZDx[i]=EUR-NZD;
         t1_EURUSDx[i]=EUR-USD;
         t1_GBPAUDx[i]=GBP-AUD;
         GBPCADx[i]=GBP-CAD;
         GBPCHFx[i]=GBP-CHF;
         GBPJPYx[i]=GBP-JPY;
         GBPNZDx[i]=GBP-NZD;
         t1_GBPUSDx[i]=GBP-USD;
         NZDCADx[i]=NZD-CAD;
         NZDCHFx[i]=NZD-CHF;
         NZDJPYx[i]=NZD-JPY;
         NZDUSDx[i]=NZD-USD;
         t1_USDCADx[i]=USD-CAD;
         t1_USDCHFx[i]=USD-CHF;
         t1_USDJPYx[i]=USD-JPY;           
     }
         ...
         for(int j=0; j<ArraySize(symbols); j++)
           {
            ...
            switch(j)
              {
               case AUDUSD:    
                  if(!MathIsValidNumber(t1_AUDUSDx[i])) break;      
                  t2_AUDUSDx[i]= iEma( t2_AUDUSDx, t1_AUDUSDx, i);
                  AUDUSDx[i] = iEma( AUDUSDx, t2_AUDUSDx, i); 
                  break;
               case EURGBP:    
                  if(!MathIsValidNumber(t1_EURGBPx[i])) break;     
                  t2_EURGBPx[i]= iEma( t2_EURGBPx, t1_EURGBPx, i);
                  EURGBPx[i] = iEma( EURGBPx, t2_EURGBPx, i); 
                  break;
               case EURJPY:    
                  if(!MathIsValidNumber(t1_EURJPYx[i])) break;      
                  t2_EURJPYx[i]= iEma( t2_EURJPYx, t1_EURJPYx, i);
                  EURJPYx[i] = iEma( EURJPYx, t2_EURJPYx, i); 
                  break;
                case EURUSD:    
/*
if(t1_EURUSDx[i] != 0.){
...
*/
                  if(!MathIsValidNumber(t1_EURUSDx[i])) break;      
                  t2_EURUSDx[i]= iEma( t2_EURUSDx, t1_EURUSDx, i);
                   EURUSDx[i] = iEma( EURUSDx, t2_EURUSDx, i); 
                  break;
                case GBPAUD:    
                  if(!MathIsValidNumber(t1_GBPAUDx[i])) break;      
                  t2_GBPAUDx[i]= iEma( t2_GBPAUDx, t1_GBPAUDx, i);
                  GBPAUDx[i] = iEma( GBPAUDx, t2_GBPAUDx, i); 
                  break;
                case GBPUSD:    
                  if(!MathIsValidNumber(t1_GBPUSDx[i])) break;      
                  t2_GBPUSDx[i]= iEma( t2_GBPUSDx, t1_GBPUSDx, i);
                  GBPUSDx[i] = iEma( GBPUSDx, t2_GBPUSDx, i); 
                  break;
                case USDCAD:    
                  if(!MathIsValidNumber(t1_USDCADx[i])) break;      
                  t2_USDCADx[i]= iEma( t2_USDCADx, t1_USDCADx, i);
                  USDCADx[i] = iEma( USDCADx, t2_USDCADx, i); 
                  break;
                case USDCHF:    
                  if(!MathIsValidNumber(t1_USDCHFx[i])) break;      
                  t2_USDCHFx[i]= iEma( t2_USDCHFx, t1_USDCHFx, i);
                  USDCHFx[i] = iEma( USDCHFx, t2_USDCHFx, i); 
                  break;
                case USDJPY:    
                  if(!MathIsValidNumber(t1_USDJPYx[i])) break;      
                  t2_USDJPYx[i]= iEma( t2_USDJPYx, t1_USDJPYx, i);
                  USDJPYx[i] = iEma( USDJPYx, t2_USDJPYx, i); 
                  break;
