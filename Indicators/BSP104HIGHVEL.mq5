...
input int                inpPeriod   =  30;           // LWMAPeriod
input int                LRPeriod    = 5;          // LRPeriod
input int                AvgPeriod   = 120;        // AvgPeriod
input int                StdPeriod   = 5000;       // StdPeriod 
input double             MultiFactor = 1.2;       // Std MultiFactor 


double SellPressure[], BuyPressure[], upStdAvgValLR[], downStdAvgValLR[], avgValLR[],
...
void OnInit()
  {
   ...
   SetIndexBuffer(4,LWMAVal,INDICATOR_CALCULATIONS);
   ...
   string short_name = "LWMAHighVel("+ (string)inpPeriod + ", " + (string)LRPeriod + ", " + 
                        (string)AvgPeriod + ", "+ (string)StdPeriod + ", "+(string)MultiFactor +")";      
...
*/       
  ...
  for(int bar=third; bar<rates_total; bar++)
     {        
       standardDeviation = StdDev(bar, StdPeriod, LWMAValLR);
       upStdAvgValLR[bar] = standardDeviation * MultiFactor;
       downStdAvgValLR[bar] = -standardDeviation * MultiFactor;
     }               

  for(int bar=fourth; bar<rates_total; bar++)
     {        
       avgValLR[bar]= iWma(bar, AvgPeriod, LWMAValLR);
     }       
