...
input int                 NLRPeriod      = 25;           // Non-LR Period
...
double p1Buffer[], p2Buffer[], p3Buffer[], p3SumBuffer[], NLRBuffer[], NLRBufferC[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(0,NLRBuffer,INDICATOR_DATA);
   SetIndexBuffer(1,NLRBufferC,INDICATOR_COLOR_INDEX);
   ...
   string shortname;
   StringConcatenate(shortname,"BuySellPowerDiffIntegralNLR(",NLRPeriod, ")" );
   IndicatorSetString(INDICATOR_SHORTNAME,shortname);

   ArrayInitialize(NLRBuffer, 0.);
...
*/
   ...
   for(int i=second; i<rates_total && !IsStopped(); i++)
     {
      NLRBuffer[i]=iNlr(p3SumBuffer[i],NLRPeriod,i,0,rates_total);
      NLRBufferC[i]=(i>0) ?(NLRBuffer[i]>NLRBuffer[i-1]) ? 2 :(NLRBuffer[i]<NLRBuffer[i-1]) ? 1 : NLRBuffer[i-1]: 0;
     }
