...
#property indicator_color1  clrGreen,clrDarkGray, clrRed
...
input int                 NlrPeriod    = 10;          // NrlPeriod
input int                 BspWmaPeriod = 10;          // BspWmaPeriod
input double              BspWmaMultiFactor = 1.;      // BspWmaMultiFactor


double SellPressure[], BuyPressure[],
       avg1SellPressure[], avg1BuyPressure[], DiffPressure[], UpWma[], DownWma[], DiffWma[],
...
void OnInit()
  {
   ...
   SetIndexBuffer(2,UpWma, INDICATOR_DATA);
   SetIndexBuffer(3,DownWma, INDICATOR_DATA);
   SetIndexBuffer(4,DiffPressure, INDICATOR_CALCULATIONS);
   SetIndexBuffer(5,DiffWma, INDICATOR_CALCULATIONS);
   ...
   string short_name = "BSPWMA("+ (string)WmaPeriod +", " + (string)NlrPeriod + ", " + (string)BspWmaPeriod + ", " + 
                                  (string)BspWmaMultiFactor  +")";      
...
int OnCalculate(const int rates_total,    // number of bars in history at the current tick
                ...
                const int &spread[])
  {
   ...
   if(prev_calculated>rates_total || prev_calculated<=0) // checking for the first start of calculation of an indicator
     {
      first=2; 
      second = first + BspWmaPeriod;
      third = first + WmaPeriod; 
     }
   else
     { 
   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       DiffPressure[bar] =( MathAbs(BuyPressure[bar]) + MathAbs(SellPressure[bar]) );
     }
       
   for(int bar=second; bar<rates_total; bar++)
     {
      DiffWma[bar] = iWma(bar, BspWmaPeriod, DiffPressure);
      UpWma[bar] = DiffWma[bar]/BspWmaPeriod*BspWmaMultiFactor;
      DownWma[bar] = DiffWma[bar]/BspWmaPeriod*BspWmaMultiFactor*(-1);
     } 
   ...
   for(int bar=third; bar<rates_total; bar++)
     {
       ...
       SumBuyPressure[bar]=avg1BuyPressure[bar] - avg1SellPressure[bar];  
     } 

   for(int i=(int)MathMax(prev_calculated-1,0); i<rates_total && !IsStopped(); i++)
     {
      SumDiffPressure[i]=iNlr(SumBuyPressure[i],NlrPeriod,i,0,rates_total);

//      if(SumDiffPressure[i]>0) SumDiffPressureC[i]=0;
//      else SumDiffPressureC[i]=1;

      if(SumDiffPressure[i]>UpWma[i]) SumDiffPressureC[i]=0;
      else if(SumDiffPressure[i]<=UpWma[i] && SumDiffPressure[i]>= DownWma[i]) SumDiffPressureC[i]=1;
      else SumDiffPressureC[i]=2;
