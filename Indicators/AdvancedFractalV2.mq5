   ...
   for(int i=first; i<rates_total && !IsStopped(); i++)
     {
      ...
      for(int n=1; n<=longframes; n++)
        {
         j=i+n;
         if(j>(rates_total-1)) j=rates_total -1;
         if(high[j]>=high[i] || high[i-n]>=high[i]) FrUpLong=false;
         if(low[j]<=low[i] || low[i-n]<=low[i]) FrDnLong=false;
        }
      //----Fractals up
      if(FrUpLong)
   ...
   for(int i=second; i<rates_total && !IsStopped(); i++)
     {
      ...
      for(int n=1; n<=shortframes; n++)
        {
         k=i+n;
         if(k>(rates_total-1)) k=rates_total -1;
         if(high[k]>=high[i] || high[i-n]>=high[i]) FrUpShort=false;
         if(low[k]<=low[i] || low[i-n]<=low[i]) FrDnShort=false;
        }
      //----Fractals up
      if(FrUpShort)
