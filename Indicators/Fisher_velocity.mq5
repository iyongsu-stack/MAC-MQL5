...
double val[],valc[],signal[],prices[],work[], velocity[];
//------------------------------------------------------------------
...
int OnInit()
{
   ...
   //
         SetIndexBuffer(0, velocity, INDICATOR_DATA);
   ...
   //---
   //
   IndicatorSetString(INDICATOR_SHORTNAME,"Fisher velocity("+(string)inpPeriod+", " +(string)signalPeriod+")");
                ...
                const int &spread[])
{
   int i= prev_calculated-1; if (i<0) i=0; for (; i<rates_total && !_StopFlag; i++)
   {
      ...
      if(i==0) velocity[i] =0.; 
      else  velocity[i] = MathArctan((signal[i]-signal[i-1])*10.) * 180./3.14159;
///      signal[i] = (i>0) ? val[i-1] : val[i];
//      valc[i]   = (val[i]>signal[i]) ? 1 :(val[i]<signal[i]) ? 2 :(i>0) ? valc[i-1]: 0;
      valc[i]   = (velocity[i]>0.) ? 1 :(velocity[i]<0.) ? 2 :(i>0) ? valc[i-1]: 0;
   }
   return (i);
}
//----------
