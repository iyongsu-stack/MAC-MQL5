...
input int                 AvgPeriod      = 4;            //Avg Period
input int                 EmaPeriod      = 7;         //EMA1 Period
input int                 SumPeriod      = 7;         //2'nd Avg Period
...
void OnInit()
  {
   ...
   SetIndexBuffer(0,SumBSPDiff,INDICATOR_DATA);   
  ...
  for(int bar=third; bar<rates_total; bar++)
     {
       ...
       BSPDiff[bar] = (dema_BuyPressure[bar] - dema_SellPressure[bar]);

     }       

  for(int bar=fourth; bar<rates_total; bar++)
     {        
       SumBSPDiff[bar]= iSum(bar, SumPeriod, BSPDiff);
     }       
...
double iSum(int end, int sumPeriod, const double &S_Array[])
{
    double sum=0.;
      
    for(int i=end+1-sumPeriod;i<=end;i++)
    {
          sum+=S_Array[i];
    }
