...
//| init                                                             |
//+------------------------------------------------------------------+
bool init(int RatesTotal,const datetime &Time[], int bar)
  {
//----
   p=period;
...
//--- the left mouse button has been pressed on the chart 
   if(id==CHARTEVENT_CLICK) 
     { 
      ...
      bar = iBarShift(Symbol(),Period(),time0+PeriodSeconds()/2,false);
//      Alert(" Subwindow: ",IntegerToString(subwindow)," Time: ",TimeToString(time,TIME_SECONDS)," Bar: ",IntegerToString(bar));
//      Alert("time: "+TimeToString(m_Time[bar], TIME_SECONDS));
      ...
      if(pn<p)
        {
         for(j=pn; j<=p; j++)
           {
            string sJ=str+")"+string(j);
            ObjectCreate(0,"_ar("+sJ,OBJ_TREND,0,m_Time[i0+1+j],0,m_Time[i0+j],0);
            ObjectSetInteger(0,"_ar("+sJ,OBJPROP_RAY,false);
            ObjectSetInteger(0,"_ar("+sJ,OBJPROP_STYLE,linesStyle);
            ObjectSetInteger(0,"_ar("+sJ,OBJPROP_WIDTH,linesWidth);

            ObjectCreate(0,"_arH("+sJ,OBJ_TREND,0,m_Time[i0+1+j],0,m_Time[i0+j],0);
            ObjectSetInteger(0,"_arH("+sJ,OBJPROP_RAY,false);
            ObjectSetInteger(0,"_arH("+sJ,OBJPROP_STYLE,linesStyle);
            ObjectSetInteger(0,"_arH("+sJ,OBJPROP_WIDTH,linesWidth);

            ObjectCreate(0,"_arL("+sJ,OBJ_TREND,0,m_Time[i0+1+j],0,m_Time[i0+j],0);
            ObjectSetInteger(0,"_arL("+sJ,OBJPROP_RAY,false);
            ObjectSetInteger(0,"_arL("+sJ,OBJPROP_STYLE,linesStyle);
            ObjectSetInteger(0,"_arL("+sJ,OBJPROP_WIDTH,linesWidth);
           }
   ...
   for(i=1; i<=nn; i++)
     {
      ...
      for(n=i0; n<=i0+p; n++)
        {
         if(i==1) sum+=m_Close[n];
         else
            sum+=m_Close[n]*MathPow(n,i-1);
        }
      b[i]=sum;
     }
   ...
   for(n=p; n>=-p/2; n--)
     {
      ...
      if(n>=0 && n<p)
        {
         ObjectMove(0,"_ar("+sN,0,m_Time[n+i0+1],fx1);
         ObjectMove(0,"_ar("+sN,1,m_Time[n+i0],fx);
         ObjectMove(0,"_arH("+sN,0,m_Time[n+i0+1],fx1+sq);
         ObjectMove(0,"_arH("+sN,1,m_Time[n+i0],fx+sq);
         ObjectMove(0,"_arL("+sN,0,m_Time[n+i0+1],fx1-sq);
         ObjectMove(0,"_arL("+sN,1,m_Time[n+i0],fx-sq);
