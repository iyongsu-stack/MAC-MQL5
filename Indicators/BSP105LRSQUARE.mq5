...
input int                 StdPeriodL    = 28800;        // Long StdPeriod
input int                 StdPeriodS    = 30;          // Short StdPeriod
input double              MultiFactorL  = 1.0;         // Long MultiFactor
input double              MultiFactorS  = 1.0;         // Short MultiFactor


double SellPressure[], BuyPressure[],
       SumSellPressure[], SumBuyPressure[], SumDiffPressure[],
       valLR[], upStdAvgValLRS[], downStdAvgValLRS[], 
       valLRS[], valLRSC[], LWMAVal[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(1,valLRSC,INDICATOR_COLOR_INDEX);  
   ...
   SetIndexBuffer(8,LWMAVal,INDICATOR_CALCULATIONS);
   ...
   for(int bar=third; bar<rates_total; bar++)
     {
      ...
      valLRSC[bar] = (bar>0) ? (valLRS[bar]>valLRS[bar-1]) ? 0 : (valLRS[bar]<valLRS[bar-1]) ? 1 : valLRS[bar-1] : 0;
     }
