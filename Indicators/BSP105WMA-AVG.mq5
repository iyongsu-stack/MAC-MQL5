...
void OnInit()
  {
   ...
   string short_name = "BSPWMAWMAABS("+ (string)AvgPeriod +", " + (string)WmaPeriod +", "+ (string)WmaPeriod2 + ")";      
   ...
   if(prev_calculated>rates_total || prev_calculated<=0) // checking for the first start of calculation of an indicator
     {
      ...
      third = second + WmaPeriod;
      fourth = third + WmaPeriod2;
     }
   else
     { 
...
*/
   ...
   for(int bar=third; bar<rates_total; bar++)
     {           
       avg1BuyPressure[bar] = iWma(bar, WmaPeriod, avgBuyPressure);
       avg1SellPressure[bar] = iWma(bar, WmaPeriod, avgSellPressure);
