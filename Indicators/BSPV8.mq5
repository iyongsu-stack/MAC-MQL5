   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       BuyPressure[bar] = tempBuyRatio * mVolume[bar]*(close[bar] - low[bar]);
       SellPressure[bar] = tempSellRatio * mVolume[bar]*(high[bar] - close[bar]);
  ...
  for(int bar=second; bar<rates_total; bar++)
     {
       ...
       CurSession = WhatIsSession(time[bar]);
       
       if( (LastSession != CurSession) && (ResetEverySession || (CurSession == AsiaSession)) )
            AvgBuyPressure2[bar] = AvgBuyPressure[bar];
       else AvgBuyPressure2[bar] = AvgBuyPressure2[bar-1] +  AvgBuyPressure[bar];

       if( (LastSession != CurSession) && (ResetEverySession || (CurSession == AsiaSession)) )
            AvgSellPressure2[bar] = AvgSellPressure[bar];
       else AvgSellPressure2[bar] = AvgSellPressure2[bar-1] +  AvgSellPressure[bar];
