...
input int                 StdPeriodS    = 6;          // StdPeriodS
...
double DiffPressure[], StdS[], DiffPressureC[], 
...
void OnInit()
  {
   ...
   SetIndexBuffer(2, StdS,INDICATOR_DATA);
   ...
   string short_name = "BSPWma("+ (string)StdPeriodS + ", "  + (string)StdPeriodL + ", " +(string)MultiFactorL1 + ", " 
...
//---- The main loop of the indicator calculation
   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       DiffPressure[bar] = MathAbs(DiffPressure[bar]);     
     }
     
   for(int bar=second; bar<rates_total; bar++)
     {
      StdS[bar]=iWma(bar, StdPeriodS, DiffPressure);
     }
