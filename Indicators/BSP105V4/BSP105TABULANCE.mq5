...
input int                 LwmaPeriod    = 5;          // WmaPeriod1
input int                 AvgPeriod    = 7;          //AvgPeriod
input int                 StdPeriodL    = 5000;        // StdPeriodL
input int                 StdPeriodS    = 2;        // StdPeriodS
input int                 AvgStdS       = 5;        // AvgStdS
input int                 StdAvgStdS    = 10;        // StdAvgS
input int                 StdAvgStdL    = 5000;      // StdAvgL
...
void OnInit()
  {
   ...
   SetIndexBuffer(3,std,INDICATOR_DATA);     
   SetIndexBuffer(4,stdSAvg,INDICATOR_CALCULATIONS);        
   ...
   SetIndexBuffer(11,avgValLR,INDICATOR_CALCULATIONS);
   SetIndexBuffer(12,stdS,INDICATOR_CALCULATIONS);     
                ...
                const int &spread[])
{
   ...
   int first, third, fourth, fifth, sixth, seventh, eighth;
   ...
   if(prev_calculated>rates_total || prev_calculated<=0) // checking for the first start of calculation of an indicator
     {
      ...
      sixth = fifth + AvgStdS;
      seventh = sixth + StdAvgStdL;
      eighth = sixth + StdAvgStdS;
     }
   else
     { 
   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       if(bar>=fourth)
       {
         double buyStd = StdDev3(bar, StdPeriodL, BuyPressure);
         if( BuyPressure[bar] > (buyStd*MaxBSPMult) ) BuyPressure[bar] = buyStd*MaxBSPMult;
         
         double sellStd = StdDev3(bar, StdPeriodL, SellPressure);
       ...
       if(bar>=sixth)
       {
          stdSAvg[bar]= iAverage(bar, AvgStdS, stdS);
       }
       
       if(bar>=seventh)
       {
          standardDeviationL = StdDev(bar, StdAvgStdL, stdSAvg, stdS);
       ...
       if(bar>=eighth)
       {
          std[bar] = StdDev(bar, StdAvgStdS,stdSAvg, stdS);
       }  
