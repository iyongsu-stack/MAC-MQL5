...
input int                 NRLPeriod1    = 3;          // WmaPeriod
input int                 NRLPeriod2    = 15;          // NRLPeriod
input int                 NRLPeriod3    = 5;          // NRLPeriod2
...
double SellPressure[], BuyPressure[],
       ...
       SumSellPressure[], SumBuyPressure[], SumDiffPressure[], SumDiffPressureC[], TempSumDiffPressure[], SumDiffPressureD[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(0,TempSumDiffPressure,INDICATOR_DATA);
   ...
   SetIndexBuffer(9,SumDiffPressureD,INDICATOR_CALCULATIONS);
   
   
  
   string short_name = "BSPWmaNrl("+ (string)NRLPeriod1 +", " + (string)NRLPeriod2 + ", " + (string)NRLPeriod3 + ")";      
...
*/
   ...
   for(int bar=second; bar<rates_total; bar++)
     {
       ...
       avg1BuyPressure[bar] = iWma(bar, NRLPeriod1, BuyPressure);
       avg1SellPressure[bar] = iWma(bar, NRLPeriod1, SellPressure);
   ...
   for(int i=(int)MathMax(prev_calculated-1,0); i<rates_total && !IsStopped(); i++)
     {
      SumDiffPressureD[i]=iNlr(SumDiffPressure[i],NRLPeriod2,i,0,rates_total);
     }


   for(int i=(int)MathMax(prev_calculated-1,0); i<rates_total && !IsStopped(); i++)
     {
      TempSumDiffPressure[i]=iNlr(SumDiffPressureD[i],NRLPeriod3,i,0,rates_total);
      SumDiffPressureC[i] = (i>0) ? (TempSumDiffPressure[i]>TempSumDiffPressure[i-1]) ? 0 : 
                                          (TempSumDiffPressure[i]<TempSumDiffPressure[i-1]) ? 1 : TempSumDiffPressure[i-1] : 0;
     }
