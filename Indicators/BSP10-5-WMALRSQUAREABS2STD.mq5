//+------------------------------------------------------------------+
//|                                   BSP10-5-WMALRSQUAREABS2STD.mq5 |
...
input int                 StdPeriodL    = 1440;        // Long StdPeriod
input int                 StdPeriodS    = 30;          // Short StdPeriod
input double              MultiFactorL  = 1.3;         // Long MultiFactor
input double              MultiFactorS  = 0.8;         // Short MultiFactor


double SellPressure[], BuyPressure[],
       SumSellPressure[], SumBuyPressure[], SumDiffPressure[],
       valLR[], upStdAvgValLRS[], downStdAvgValLRS[], 
       valLRS[], LWMAVal[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(1,upStdAvgValLRS,INDICATOR_DATA); 
   SetIndexBuffer(2,downStdAvgValLRS,INDICATOR_DATA);  
   SetIndexBuffer(3,valLR,INDICATOR_CALCULATIONS);
   SetIndexBuffer(4,SumDiffPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(5,SumBuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(6,SumSellPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(7,LWMAVal,INDICATOR_CALCULATIONS);
   SetIndexBuffer(8,BuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(9,SellPressure,INDICATOR_CALCULATIONS);
  
   string short_name = "BSPLWMALRSQUAREDABSSTD( "+ (string)LwmaPeriod + ", " + (string)LRPeriod + ", " + (string)LRSPeriod + ", "+ 
                                          (string)StdPeriodL + ", " + (string)StdPeriodS + ", " + (string)MultiFactorL + ", " +(string)MultiFactorS +")";      
...
//---- The main loop of the indicator calculation
  ...
  for(int bar=fourth; bar<rates_total; bar++)
     {        
       standardDeviationL = StdDev(bar, StdPeriodL, valLRS);
       standardDeviationS = StdDev(bar, StdPeriodS, valLRS);

       if( (standardDeviationL*MultiFactorL) >= (standardDeviationS*MultiFactorS) )
         {
            upStdAvgValLRS[bar] =  standardDeviationL * MultiFactorL;
            downStdAvgValLRS[bar] = (-1) * standardDeviationL * MultiFactorL;
         }
      else
         {
            upStdAvgValLRS[bar] = standardDeviationS * MultiFactorS;
            downStdAvgValLRS[bar] = (-1) * standardDeviationS * MultiFactorS;
         }          
     }   
