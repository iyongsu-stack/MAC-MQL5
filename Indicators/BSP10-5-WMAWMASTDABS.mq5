...
input int                 StdAverage   = 30;          //StdAverage
...
double SellPressure[], BuyPressure[],
       ...
       stdAvgVal[], upStdAvgVal[], downStdAvgVal[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(0,upStdAvgVal,INDICATOR_DATA);
   SetIndexBuffer(1,downStdAvgVal,INDICATOR_DATA);
   SetIndexBuffer(2,WmaSumDiffPressure,INDICATOR_DATA);
   SetIndexBuffer(3,SumDiffPressure,INDICATOR_DATA);
   SetIndexBuffer(4,SumDiffPressureC,INDICATOR_COLOR_INDEX);   
   SetIndexBuffer(5,stdAvgVal,INDICATOR_CALCULATIONS);   
   ...
   string short_name = "BSPWMAWMASTDABS("+ (string)WmaPeriod +", "+ (string)WmaPeriod2 +", "+ (string)StdAverage + ", " + (string)StdPeriod + ")";      
...
*/
  ...
  for(int bar=fourth; bar<rates_total; bar++)
     {        
       stdAvgVal[bar]= iWma(bar, StdAverage, WmaSumDiffPressure);
     }       

  for(int bar=fifth; bar<rates_total; bar++)
     {        
       standardDeviation = StdDev(bar, StdPeriod, stdAvgVal, SumDiffPressure);
       upStdAvgVal[bar] = WmaSumDiffPressure[bar] + standardDeviation * MultiFactor;
       downStdAvgVal[bar] = WmaSumDiffPressure[bar] - standardDeviation * MultiFactor;
     }   
