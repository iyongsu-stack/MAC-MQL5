...
double UpBuffer[], mtf_UpBuffer[];
double DnBuffer[], mtf_DnBuffer[];
double IndBuffer[], mtf_IndBuffer[];
...
//--- determining the accuracy of displaying the indicator values
   ...
   if(TimeFrame > 0) 
   {
      mtf_handle = iCustom(Symbol(),TimeFrame,"MaRoundChannelMTF",TimeFrame,XMA_Method,XLength,XPhase,IPC,
                           MaRound, ATRPeriod, ATR_Factor, ChanContinuity, Shift);
//---- getting the ATR indicator handle
     ATR_Handle=iATR(NULL,TimeFrame,ATRPeriod);
...
//---- Declaration of integer variables
   int to_copy,first,bar, mtflimit, shift, x, y, copied;
...
//--- the main loop of calculations
   if(tf > Period())
   {
      ...
      for(shift=0,y=0;shift<mtflimit;shift++)
      {
         ...
         copied = CopyBuffer(mtf_handle,0,mtf_time,mtf_time,mtf_IndBuffer);
         if(copied <= 0) return(0);
      
         copied = CopyBuffer(mtf_handle,1,mtf_time,mtf_time,mtf_UpBuffer);
         if(copied <= 0) return(0) ;
  
         copied = CopyBuffer(mtf_handle,2,mtf_time,mtf_time,mtf_DnBuffer);
         if(copied <= 0) return(0);
      
         IndBuffer[x] = mtf_IndBuffer[0];
         UpBuffer[x] = mtf_UpBuffer[0];
         DnBuffer[x] = mtf_DnBuffer[0];      
      }
   }
   else
   {
