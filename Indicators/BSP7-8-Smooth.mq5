...
input double              EmaSensitivity = 2.;          //2'nd Avg sensitivity
input int                 EmaPeriod2     = 70;         //2'nd Avg Period
input int                 StdPeriod      = 1440;         //Std Period 
input double              StdMultiFactor = 0.5;        //Std Multi FActor                
...
double alpha2 = EmaSensitivity / (1.0+MathSqrt(EmaPeriod2));
...
//---- The main loop of the indicator calculation
  ...
  for(int bar=third; bar<rates_total; bar++)
     {
       ...
       ema_BSPDiff  = XMA5.XMASeries(third,prev_calculated,rates_total,(int)MODE_EMA,
                                    XPhase2,XLength3,temp_BSPDiff, bar,false);
       dema_BSPDiff = XMA6.XMASeries(third,prev_calculated,rates_total,(int)MODE_EMA,
                                    XPhase2,XLength3,ema_BSPDiff,bar,false);
       BSPDiff[bar] = XMA7.XMASeries(third,prev_calculated,rates_total,(int)MODE_EMA,
  ...
  for(int bar=fourth; bar<rates_total; bar++)
     {        
       ...
       AvgBSPDiff[bar]= iEma(AvgBSPDiff, BSPDiff, bar,  alpha2);
     }       
