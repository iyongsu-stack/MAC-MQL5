...
input int                 NlrPeriod    = 60;         // NlrPeriod

double NlrDiffPressure[], NlrDiffPressureC[];
...
void OnInit()
  {
   ...
   string short_name = "CloseNlr("+(string)NlrPeriod + ")";      
...
int OnCalculate(const int rates_total,    // number of bars in history at the current tick
                ...
                const int &spread[])
  {
   ...
   for(int i=(int)MathMax(prev_calculated-1,0); i<rates_total && !IsStopped(); i++)
     {
      NlrDiffPressure[i]=iNlr(close[i],NlrPeriod,i,0,rates_total);
