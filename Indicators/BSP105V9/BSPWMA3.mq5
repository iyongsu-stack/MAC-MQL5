...
#property indicator_color1  clrPowderBlue,clrPaleGoldenrod
#property indicator_style1  STYLE_SOLID
#property indicator_width1  2

#property indicator_type2   DRAW_COLOR_LINE
#property indicator_color2  clrKhaki,clrViolet
...
input int                 WmaPeriod2   = 10;         // wmaPeriod2
input int                 WmaPeriod3   = 20;         // wmaPeriod3

double DiffPressure[], 
       DiffPressure1[], DiffPressureC1[],
       DiffPressure2[], DiffPressureC2[],
       DiffPressure3[], DiffPressureC3[] ;
...
void OnInit()
  {
   ...
   ArrayInitialize(DiffPressure2,0.0);
   ArrayInitialize(DiffPressure3,0.0);
   ArrayInitialize(DiffPressureC1,0); 
   ArrayInitialize(DiffPressureC2,0);
   ArrayInitialize(DiffPressureC3,0);

   SetIndexBuffer(0,DiffPressure1,INDICATOR_DATA);
   SetIndexBuffer(1,DiffPressureC1,INDICATOR_COLOR_INDEX);
   SetIndexBuffer(2,DiffPressure2,INDICATOR_DATA);
   SetIndexBuffer(3,DiffPressureC2,INDICATOR_COLOR_INDEX);
   SetIndexBuffer(4,DiffPressure3,INDICATOR_DATA);
   SetIndexBuffer(5,DiffPressureC3,INDICATOR_COLOR_INDEX);
   SetIndexBuffer(6,DiffPressure,INDICATOR_CALCULATIONS);
  
   string short_name = "BSPTEST("+ (string)WmaPeriod1 + 
                       "," + (string)WmaPeriod2 + 
                       "," + (string)WmaPeriod3 + 
                       ")";      
   ...
   if(prev_calculated>rates_total || prev_calculated<=0) // checking for the first start of calculation of an indicator
     {
      ...
      second = start + WmaPeriod2; 
      third = start + WmaPeriod3; 
     }
   else
     { 
   ...
   for(int bar=second; bar<rates_total; bar++)
     {
       ...
       DiffPressure2[bar] = iWma(bar, WmaPeriod2, DiffPressure);

       DiffPressureC2[bar] = (bar>0) ? (DiffPressure2[bar]>=DiffPressure2[bar-1]) ? 0 : 
                                          (DiffPressure2[bar]<DiffPressure2[bar-1]) ? 1 : DiffPressure2[bar-1] : 0;
     } 

   for(int bar=third; bar<rates_total; bar++)
     {
            
       DiffPressure3[bar] = iWma(bar, WmaPeriod3, DiffPressure);

       DiffPressureC3[bar] = (bar>0) ? (DiffPressure3[bar]>=DiffPressure3[bar-1]) ? 0 : 
                                          (DiffPressure3[bar]<DiffPressure3[bar-1]) ? 1 : DiffPressure3[bar-1] : 0;
     } 
