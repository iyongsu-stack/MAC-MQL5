...
double BuyRatio[], SellRatio[], DiffRatio[], WmaDiffRatio[], WmaDiffRatioC[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(1,WmaDiffRatioC,INDICATOR_COLOR_INDEX);
   SetIndexBuffer(2,BuyRatio,INDICATOR_CALCULATIONS);
   SetIndexBuffer(3,SellRatio,INDICATOR_CALCULATIONS);
   SetIndexBuffer(4,DiffRatio,INDICATOR_CALCULATIONS);   
  
   string short_name = "BSPRatio("+ (string)WmaPeriod + ")";      
   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       {
         BuyRatio[bar] = tempBuyRatio/tempTotalPressure;
         SellRatio[bar] = -tempSellRatio/tempTotalPressure;       
       }

       DiffRatio[bar] = BuyRatio[bar] + SellRatio[bar];
     }
   ...
   for(int bar=second; bar<rates_total; bar++)
     {
       ...
       WmaDiffRatio[bar] = iWma(bar, WmaPeriod, DiffRatio);
       WmaDiffRatioC[bar] = (bar>0) ? (WmaDiffRatio[bar]>WmaDiffRatio[bar-1]) ? 0 : 
