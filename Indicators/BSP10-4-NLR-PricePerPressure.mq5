//+------------------------------------------------------------------+
//|                                 BSP10-4-NLR-PricePerPressure.mq5 |
//|                                                     Yong-su, Kim |
...
double SellPressure[], BuyPressure[],
       ...
       val[], valc[], Pressure[], PricePerPressure[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(0,PricePerPressure,INDICATOR_DATA);
...
int OnCalculate(const int rates_total,    // number of bars in history at the current tick
                ...
                const int &spread[])
  {
   ...
   double mVolume, firstOpenPrice=0., priceGap;
   ...
   for(int bar=third; bar<rates_total; bar++)
     {
      ...
      if( ((LastTrend == UpTrend) && (CurTrend == DownTrend)) || ((LastTrend == DownTrend) && (CurTrend == UpTrend) )) 
      {
         ...
         firstOpenPrice = open[bar];
         priceGap = MathAbs(close[bar] - open[bar]);
         if(Pressure[bar] == 0.) 
         {
            PricePerPressure[bar] = PricePerPressure[bar-1];
            continue;
         }
         else PricePerPressure[bar] = priceGap/Pressure[bar];
         
      }   
      else 
      {
         Pressure[bar] = Pressure[bar-1] + (BuyPressure[bar] - SellPressure[bar]);
         priceGap = MathAbs(close[bar] - firstOpenPrice);
         if(Pressure[bar] == 0.) 
         {
            PricePerPressure[bar] = PricePerPressure[bar-1];
            continue;
