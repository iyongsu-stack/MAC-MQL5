   ...
   for(int bar=fourth; bar<rates_total; bar++)
     {
          ...
          standardDeviationL = StdDev(bar, StdL, DiffPressure);
...
{
    double StdValue=0., dAmount=0.;  
    static int lastBar=0;
    static double lastAmount, preValue=0.;
    ...
    if(firstCalculation)
      {
       ...
       {
         int k = end - SDPeriod;
         dAmount = lastAmount - (S_Array[k])*(S_Array[k]) + (S_Array[end])*(S_Array[end]); 
         preValue = dAmount;                                              
       }       
