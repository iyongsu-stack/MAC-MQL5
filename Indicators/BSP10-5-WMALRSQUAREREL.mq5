...
input int                 LwmaPeriod    = 30;          // WmaPeriod1
input int                 LRPeriod     = 5;           // LRPeriod
input int                 LRSPeriod    = 5;           // LRSPeriod
input int                 AvgSPeriod    = 1440;          // AvgSPeriod
input int                 StdSPeriod    = 1440;        // S StdPeriod
input double              SMultiFactor  = 1.5;         // S StdMultiFactor

double SellPressure[], BuyPressure[],
       SumSellPressure[], SumBuyPressure[], SumDiffPressure[],
       valLR[], avgValLRS[], stdAvgValLRS[], upStdAvgValLRS[], downStdAvgValLRS[], 
       valLRS[], LWMAVal[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(0,valLRS,INDICATOR_DATA);
   SetIndexBuffer(1,avgValLRS,INDICATOR_DATA);
   SetIndexBuffer(2,upStdAvgValLRS,INDICATOR_DATA); 
   SetIndexBuffer(3,downStdAvgValLRS,INDICATOR_DATA);  
   SetIndexBuffer(4,valLR,INDICATOR_CALCULATIONS);
   SetIndexBuffer(5,stdAvgValLRS,INDICATOR_CALCULATIONS);
   SetIndexBuffer(6,SumDiffPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(7,SumBuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(8,SumSellPressure,INDICATOR_CALCULATIONS);
   ...
   string short_name = "BSPLWMALRSQUAREDREL("+ (string)LwmaPeriod + ", " + (string)LRPeriod + ", " + (string)LRSPeriod + ", "+ 
                                          (string)AvgSPeriod + ", " + (string)StdSPeriod + ", " + (string)SMultiFactor +")";      
   ...
   if(prev_calculated>rates_total || prev_calculated<=0) // checking for the first start of calculation of an indicator
     {
      ...
      third = second + LRSPeriod;
      fourth = third + AvgSPeriod;
      fifth = third + StdSPeriod; 
      sixth  = fifth + StdSPeriod;
     }
   else
     { 
   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       tempTotalPressure=tempBuyRatio + tempSellRatio;
       if(tempTotalPressure == 0.) tempTotalPressure = 0.000001;
...
*/       
   ...
   for(int bar=third; bar<rates_total; bar++)
     {
      valLRS[bar]=LinearRegression(bar, LRSPeriod, valLR);
     }


  for(int bar=fourth; bar<rates_total; bar++)
     {        
       avgValLRS[bar]= iWma(bar, AvgSPeriod, valLRS);
     }       

  for(int bar=fifth; bar<rates_total; bar++)
     {        
       stdAvgValLRS[bar]= iAverage(bar, StdSPeriod, valLRS);
     }       

  for(int bar=sixth; bar<rates_total; bar++)
     {        
       standardDeviation = StdDev(bar, StdSPeriod, stdAvgValLRS, valLRS);
       upStdAvgValLRS[bar] = avgValLRS[bar] + standardDeviation * SMultiFactor;
       downStdAvgValLRS[bar] = avgValLRS[bar] - standardDeviation * SMultiFactor;
     }               
