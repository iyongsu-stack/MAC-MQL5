...
input int                 WmaPeriod    = 2;          // wmaPeriod
...
void OnInit()
  {
   ...
   ArrayInitialize(TempSumDiffPressure,0.0);
   ...
   SetIndexBuffer(8,TempSumDiffPressure,INDICATOR_CALCULATIONS);
   
  
   string short_name = "BSPNlr("+ (string)WmaPeriod + ", "+(string)NlrPeriod + ")";      
...
*/
   ...
   for(int bar=second; bar<rates_total; bar++)
     {
       ...
       TempSumDiffPressure[bar]=SumBuyPressure[bar] - SumSellPressure[bar];  
   ...
   for(int i=(int)MathMax(prev_calculated-1,0); i<rates_total && !IsStopped(); i++)
     {
      SumDiffPressure[i]=iNlr(TempSumDiffPressure[i],NlrPeriod,i,0,rates_total);
      SumDiffPressureC[i] = (i>0) ? (SumDiffPressure[i]>SumDiffPressure[i-1]) ? 0 : 
                                          (SumDiffPressure[i]<SumDiffPressure[i-1]) ? 1 : SumDiffPressure[i-1] : 0;
     }
