...
#property indicator_label1  "VADX"
...
//--- indicator buffers
   ...
   SetIndexBuffer(1,ExtPDIBuffer,INDICATOR_CALCULATIONS);
   SetIndexBuffer(2,ExtNDIBuffer,INDICATOR_CALCULATIONS);
...
//--- indicator short name
   string short_name="VADx("+string(ExtADXPeriod)+")";
   ...
   for(int i=start; i<rates_total && !IsStopped(); i++)
     {
      ...
      if(tmp!=0.0)
         tmp=100.0*(ExtPDIBuffer[i]-ExtNDIBuffer[i])/tmp;
      else
         tmp=0.0;
      ExtTmpBuffer[i]=tmp;
      //--- fill smoothed ADX buffer
      ExtADXBuffer[i]=ExponentialMA(i,MA_Period,ExtADXBuffer[i-1],ExtTmpBuffer);
     }
