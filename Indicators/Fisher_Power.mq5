...
#property indicator_label1  "Fisher Velocity"
...
double val[],valc[],signal[],prices[],work[], velocity[], power[];
//------------------------------------------------------------------
...
int OnInit()
{
   ...
   //
         ...
         SetIndexBuffer(2,velocity, INDICATOR_CALCULATIONS);
         SetIndexBuffer(3,val   ,INDICATOR_CALCULATIONS);
         SetIndexBuffer(4,signal,INDICATOR_CALCULATIONS);
         SetIndexBuffer(5,prices,INDICATOR_CALCULATIONS);
         SetIndexBuffer(6,work  ,INDICATOR_CALCULATIONS);
   //
   //---
   //
   IndicatorSetString(INDICATOR_SHORTNAME,"Fisher Power("+(string)inpPeriod+", " +(string)signalPeriod+")");
...
//      valc[i]   = (val[i]>signal[i]) ? 1 :(val[i]<signal[i]) ? 2 :(i>0) ? valc[i-1]: 0;
      valc[i]   = (power[i]>0.) ? 1 :(power[i]<0.) ? 2 :(i>0) ? valc[i-1]: 0;
   }
   return (i);
}
//----------
