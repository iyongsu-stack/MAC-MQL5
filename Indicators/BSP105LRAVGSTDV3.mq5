...
input int                 StdAvgPeriodS  = 10;        // StdAvgPeriodS
...
void OnInit()
  {
   ...
   SetIndexBuffer(8,stdSAvg, INDICATOR_DATA);  
   SetIndexBuffer(9,SumDiffPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(10,SumBuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(11,SumSellPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(12,LWMAVal,INDICATOR_CALCULATIONS);
   SetIndexBuffer(13,BuyPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(14,SellPressure,INDICATOR_CALCULATIONS);
   SetIndexBuffer(15,avgValLR,INDICATOR_CALCULATIONS);
   
  
   string short_name = "BSPLRAVGSTD("+ (string)LwmaPeriod + ", "  + (string)AvgPeriod + ", " +
                                            (string)StdPeriodL + ", " + (string)StdPeriodS + ", " +  (string)StdAvgPeriodS + ", " + 
...
int OnCalculate(const int rates_total,    // number of bars in history at the current tick
                ...
                const int &spread[])
  {
   ...
   int first, third, fourth, fifth, sixth;
...
//---- The main loop of the indicator calculation
  ...
  for(int bar=sixth; bar<rates_total; bar++)
     {        
      stdSAvg[bar] = iWma(bar, StdAvgPeriodS, stdS);
     }  
   ...
   for(int bar=fourth; bar<rates_total; bar++)
     {
      ...
      up1StdAvgValLR[bar]   =  stdSAvg[bar] + standardDeviationL * MultiFactorL1;
      down1StdAvgValLR[bar] =  stdSAvg[bar] - standardDeviationL * MultiFactorL1;

      up2StdAvgValLR[bar]   =  stdSAvg[bar] + standardDeviationL * MultiFactorL2;
      down2StdAvgValLR[bar] =  stdSAvg[bar] - standardDeviationL * MultiFactorL2;

      up3StdAvgValLR[bar]   =  stdSAvg[bar] + standardDeviationL * MultiFactorL3;
      down3StdAvgValLR[bar] =  stdSAvg[bar] - standardDeviationL * MultiFactorL3;
