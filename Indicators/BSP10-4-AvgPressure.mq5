...
input int                 WmaPeriod    = 6;          // EmaPeriod1
...
int OnCalculate(const int rates_total,    // number of bars in history at the current tick
                ...
                const int &spread[])
  {
   ...
   int first, second, third, NumBar=0;
   double mVolume;
...
//---- The main loop of the indicator calculation
   for(int bar=first; bar<rates_total; bar++)
     ...
     for(int bar=second; bar<rates_total; bar++)
     {
       ...
       avgBuyPressure[bar] = iWma(bar, WmaPeriod, BuyPressure);
       avgSellPressure[bar] = iWma(bar, WmaPeriod, SellPressure);
   ...
   for(int bar=third; bar<rates_total; bar++)
     {
      ...
      if( ((LastTrend == UpTrend) && (CurTrend == DownTrend)) || ((LastTrend == DownTrend) && (CurTrend == UpTrend) )) 
      {
         ...
         NumBar = 1;
         avgPressure[bar] = Pressure[bar];
      }   
      else 
      {
         NumBar++;
         Pressure[bar] = Pressure[bar-1] + (BuyPressure[bar] - SellPressure[bar]);
         avgPressure[bar] = Pressure[bar]/NumBar;
      }     
