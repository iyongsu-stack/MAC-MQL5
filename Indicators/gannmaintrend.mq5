...
#property description "Main trend indicator"
...
//---------------------------------------------------------------------
string InfoTitles_Array[]={ "Last Main Top:","Last Main Low:" };
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"GannMainTrend");
   PlotIndexSetString(0,PLOT_LABEL,"GannMainTrend");
   ...
   for(int i=start; i<rates_total; i++)
     {
      ...
      if(trend_dir==0)
        {
         if(high[i]>high[i-1] && high[i-1]>high[i-2] && high[i-2]>high[i-3]/*&& low[ i ] > low[ index - 1 ] && low[ index - 1 ] > low[ index - 2 ]*/)
           {
            trend_dir=1;
            active_bar=i;
           }
         else if(/*high[ i ] < high[ i - 1 ] && high[ i - 1 ] < high[ i - 2 ] &&*/ low[i]<low[i-1] && low[i-1]<low[i-2] && low[i-2]<low[i-3])
           {
            trend_dir=-1;
            active_bar=i;
            last_top_bar = -1;
      ...
      if(trend_dir==1)
        {
         ...
         if(low[i]<low[i-1] && low[i-1]<low[i-2] && low[i-2]<low[active_bar])
           {
            ColorBuffer[ active_bar ]= 0;
            DataBuffer[ active_bar ] = high[ active_bar ];
      ...
      if(trend_dir==-1)
        {
         ...
         if(high[i]>high[i-1] && high[i-1]>high[i-2] && high[i-2]>high[active_bar])
           {
            ColorBuffer[ active_bar ]= 1;
            DataBuffer[ active_bar ] = low[ active_bar ];
