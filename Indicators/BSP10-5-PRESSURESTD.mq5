...
input int                 WmaPeriod    = 30;          // wmaPeriod
input int                 StdPeriod   = 2880;          // StdPeriod
input double              MultiFactor = 1.0;           // StdMultiFactor


double TotalPressure[],
       avgTotalPressure[], upStdTotalPressure[];

//+------------------------------------------------------------------+  
void OnInit()
  {

   SetIndexBuffer(0,avgTotalPressure,INDICATOR_DATA);
   SetIndexBuffer(1,upStdTotalPressure,INDICATOR_DATA);
   SetIndexBuffer(2,TotalPressure,INDICATOR_CALCULATIONS);
  
   string short_name = "BSPPressureStd("+ (string)WmaPeriod +", "+ (string)StdPeriod + ", "+   (string)MultiFactor + ")";      
...
//---- The main loop of the indicator calculation
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       TotalPressure[bar] = (MathAbs(tempBuyRatio)+MathAbs(tempSellRatio) )*100.;
   ...
   for(int bar=second; bar<rates_total; bar++)
     {            
       avgTotalPressure[bar] = iWma(bar, WmaPeriod, TotalPressure);
     } 

  for(int bar=third; bar<rates_total; bar++)
     {        
       standardDeviation = StdDev(bar, StdPeriod, TotalPressure);
       upStdTotalPressure[bar] = standardDeviation * MultiFactor;
     }               
