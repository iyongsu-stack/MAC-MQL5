...
input double         InpMultiFactor = 1.0;       //Channel Width(SD*Multi)
...
//--- check input variables
   ...
   SetIndexBuffer(5,highBuffer,INDICATOR_CALCULATIONS);
   SetIndexBuffer(6,lowBuffer,INDICATOR_CALCULATIONS);
...
int OnCalculate(const int rates_total,
  ...
  {
//---
   double centerLRa, highLRa, lowLRa;
...
//       PlotIndexSetInteger(5,PLOT_DRAW_BEGIN,rates_total-ExChPeriod-1);
        ...
        if(HowToChannel == byClose){
         ...
         highLRa = LinearRegression(rates_total, high, highBuffer);
         StdDev(rates_total, highLRa, high, highBuffer, upBuffer, InpMultiFactor);
         lowLRa = LinearRegression(rates_total, low, lowBuffer);                
         StdDev(rates_total, lowLRa, low, lowBuffer, downBuffer, -1*InpMultiFactor);
        }
