...
input int                 AveragePeriod = 30;           // Average Period
input int                 AveragePeriod1 = 240;           // Average Period1
...
double p1Buffer[], p2Buffer[], p3Buffer[], /*, p1BufferLR[], p2BufferLR[], p3BufferLR[]*/
       p1AvgBuffer[], p2AvgBuffer[], p3AvgBuffer[], pPercentBuffer[], 
       p1AvgBuffer1[], p2AvgBuffer1[], p3AvgBuffer1[], pPercentBuffer1[];
...
//   SetIndexBuffer(2,p3BufferLR,INDICATOR_DATA);
   SetIndexBuffer(0,pPercentBuffer,INDICATOR_DATA); 
   SetIndexBuffer(1,pPercentBuffer1,INDICATOR_DATA);   
  
   SetIndexBuffer(2,p1AvgBuffer,INDICATOR_CALCULATIONS);
   SetIndexBuffer(3,p2AvgBuffer,INDICATOR_CALCULATIONS);
   SetIndexBuffer(4,p3AvgBuffer,INDICATOR_CALCULATIONS);
   SetIndexBuffer(5,p1AvgBuffer1,INDICATOR_CALCULATIONS);
   SetIndexBuffer(6,p2AvgBuffer1,INDICATOR_CALCULATIONS);
   SetIndexBuffer(7,p3AvgBuffer1,INDICATOR_CALCULATIONS);

   SetIndexBuffer(8,p1Buffer,INDICATOR_CALCULATIONS);
   SetIndexBuffer(9,p2Buffer,INDICATOR_CALCULATIONS);
   SetIndexBuffer(10,p3Buffer,INDICATOR_CALCULATIONS);
   
//   ArrayInitialize(p1BufferLR, 0.);
//   ArrayInitialize(p2BufferLR, 0.);
//   ArrayInitialize(p3BufferLR, 0.);
   ArrayInitialize(pPercentBuffer, 0.);
   ArrayInitialize(pPercentBuffer1, 0.);

//   ArrayInitialize(mPercentBuffer, 0.);

   ArrayInitialize(p1AvgBuffer, 0.);
   ArrayInitialize(p2AvgBuffer, 0.);
   ArrayInitialize(p3AvgBuffer, 0.);
   ArrayInitialize(p1AvgBuffer1, 0.);
   ArrayInitialize(p2AvgBuffer1, 0.);
   ArrayInitialize(p3AvgBuffer1, 0.);
   ...
   if(prev_calculated>rates_total || prev_calculated<=0) // checking for the first start of calculation of an indicator
     {
      ...
      forth =   rates_total-first + AveragePeriod1;
     }
   else
     {     
   ...
   for(int avgbar=third; avgbar<rates_total; avgbar++)
     {
        ...
        pPercentBuffer[avgbar] = 100.*p1AvgBuffer[avgbar]/(p1AvgBuffer[avgbar]+MathAbs(p2AvgBuffer[avgbar]));
//        mPercentBuffer[avgbar] = -100.*p2AvgBuffer[avgbar]/(p1AvgBuffer[avgbar]+MathAbs(p2AvgBuffer[avgbar]));                
     }

   for(int avgbar=forth; avgbar<rates_total; avgbar++)
     {
 
        p1AvgBuffer1[avgbar]= Average(avgbar, AveragePeriod1, p1Buffer); 
        p2AvgBuffer1[avgbar]= Average(avgbar, AveragePeriod1, p2Buffer); 
        p3AvgBuffer1[avgbar]= Average(avgbar, AveragePeriod1, p3Buffer); 
        pPercentBuffer1[avgbar] = 100.*p1AvgBuffer1[avgbar]/(p1AvgBuffer1[avgbar]+MathAbs(p2AvgBuffer1[avgbar]));
