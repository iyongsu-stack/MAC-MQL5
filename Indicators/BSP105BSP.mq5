...
double TotalPressure[], WmaTotalPressure[];
...
void OnInit()
  {
   ...
   SetIndexBuffer(0,TotalPressure,INDICATOR_DATA);
   SetIndexBuffer(1,WmaTotalPressure,INDICATOR_DATA);
   

   string short_name = "BSPWMA("+ (string)WmaPeriod + ", " + (string)MultiRatio +  ")";      
   ...
   for(int bar=first; bar<rates_total; bar++)
     {
       ...
       TotalPressure[bar] =( MathAbs(tempBuyRatio) + MathAbs(tempSellRatio) ) * 10000./2.;
     }  

   for(int bar=second; bar<rates_total; bar++)
     {
      WmaTotalPressure[bar]=iWma(bar, WmaPeriod, TotalPressure)*MultiRatio;
     }
