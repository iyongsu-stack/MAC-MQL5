...
//+------------------------------------------------------------------+
bool SolveLots(int m_Session, double & aLots)
{
   if(Lots==0){
      if(PositionSummary[m_Session].totalNumPositions == 0) aLots=fLotsNormalize(AccountInfoDouble(ACCOUNT_BALANCE)*MaximumRisk*1./1000.0);
      else aLots=fLotsNormalize(AccountInfoDouble(ACCOUNT_BALANCE)*MaximumRisk*1./1000.0*LotsMultiFactor);
      return(true);        
   }else{
      if(PositionSummary[m_Session].totalNumPositions == 0) aLots=Lots; 
