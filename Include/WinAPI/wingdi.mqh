...
int          DrawEscape(HANDLE hdc,int escape,int in_len,uchar &in[]);
...
int          Escape(HANDLE hdc,int escape,int in_len,uchar &in[],PVOID out);
...
int          GdiAlphaBlend(HANDLE dest,int dest_x,int dest_y,int dest_w,int dest_h,HANDLE src,int src_x,int src_y,int src_w,int src_h,BLENDFUNCTION &ftn);
int          GdiComment(HANDLE hdc,uint size,const uchar &data[]);
int          GdiFlush(void);
uint         GdiGetBatchLimit(void);
int          GdiGradientFill(HANDLE hdc,TRIVERTEX &vertex[],uint vertex_count,PVOID mesh,uint count,uint mode);
uint         GdiSetBatchLimit(uint dw);
int          GdiTransparentBlt(HANDLE dest,int dest_x,int dest_y,int dest_w,int dest_h,HANDLE src,int src_x,int src_y,int src_w,int src_h,uint transparent);
...
uint         GetFontData(HANDLE hdc,uint table,uint offset,uchar& buffer[],uint buffer_len);
...
uint         GetGlyphOutlineW(HANDLE hdc,uint symbol,uint format,GLYPHMETRICS &lpgm,uint buffer_len,uchar &buffer[],MAT2 &lpmat2);
...
int          GetTextExtentExPointW(HANDLE hdc,const string str,int str_len,int max_extent,int &fit,int &dx[],SIZE &size);
...
uint         GetWinMetaFileBits(HANDLE hemf,uint data16_len,uchar &data16[],int map_mode,HANDLE ref);
...
int          MaskBlt(HANDLE dest,int dest_x,int dest_y,int width,int height,HANDLE src,int src_x,int src_y,HANDLE mask,int mask_x,int mask_y,uint rop);
...
int          PlgBlt(HANDLE dest,POINT &point,HANDLE src,int src_x,int src_y,int width,int height,HANDLE mask,int mask_x,int mask_y);
...
int          SetDIBitsToDevice(HANDLE hdc,int dest_x,int dest_y,uint w,uint h,int src_x,int src_y,uint StartScan,uint lines,uchar &bits[],BITMAPINFO &lpbmi,uint ColorUse);
...
int          StretchBlt(HANDLE dest,int dest_x,int dest_y,int dest_w,int dest_h,HANDLE src,int src_x,int src_y,int src_w,int src_h,uint rop);
int          StretchDIBits(HANDLE hdc,int dest_x,int dest_y,int DestWidth,int DestHeight,int src_x,int src_y,int SrcWidth,int SrcHeight,uchar &bits[],BITMAPINFO &lpbmi,uint usage,uint rop);
