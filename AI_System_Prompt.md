...
### 부가 원칙
- **🎯 확정된 전략 구조 (2026-03-07 갱신)**: AI 학습 → TrailingStop 청산
  ```
  AI:      진입 여부 전부 480개 피처(BSPScale 포함)로 AI가 학습하여 결정
  청산:    TrailingStopVx 전담 (고정 TP 사용 안 함)
  ```
- **ATR 동적 배리어 라벨링 선행 필수**: 전체 봉에 대해 라벨 생성. 배리어: TP=ATR×2.5 / SL=ATR×2.5 / 30봉 / TP판정=Close기준. **실전 청산은 TrailingStopVx 전담** (AI는 진입만 학습).
