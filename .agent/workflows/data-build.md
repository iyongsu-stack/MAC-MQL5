...
> 3. **절대값 사용 금지**: `build_tech_derived.py`에서 파생 피처(Z-score/Slope/Ratio/MA비율)로 자동 변환. 스케일 오실레이터(0~100)는 Passthrough 허용
...
> **출력**: `tech_features_derived.parquet` (~72개 파생 피처)
> **핵심 로직**:
> - Z-score: `x.shift(1).rolling(W)` (Shift+1 원칙, W=60/240/1440)
...
> - TickVolume: MA비율(60/240/1440) + Z-Score(60/240/1440) 병행 (원본 DROP)
> - OHLC 원본: DROP
...
> 5. **원본 절대값 잔존 검증**: FORBIDDEN_COLS(OHLC, TickVolume 등) 누수 여부
...
Yahoo/FRED → macro_features.parquet ─┤
                           ...
                           Step 1: build_tech_derived.py
                                     │  (Z-score Shift+1, MTF 변화점, CE Ratio, TV MA비율)
                                     ▼
                           tech_features_derived.parquet (~72 cols)
                                     │
                           ...
                           Step 3: merge_features.py
                                     │  (Macro Shift+1, merge_asof, Label Join, dropna)
                                     ▼
                           AI_Study_Dataset.parquet (~460 cols)
                                     │
                           Step 4: verify_merged_dataset.py
                                     │  (Shift+1, MTF, Label, 절대값 잔존 검증)
                                     ▼
                           ✅ AI 학습 준비 완료
```
