...
> ```
> **시뮬레이션 범위**: 전체 봉에 대해 라벨 생성 및 시뮬레이션 수행.
...
### 설정 (표준 배리어, 2026-03-07 갱신)
```
TP:     ATR(14) × 2.5  (대칭 R:R=1.0, 충분한 수익폭 확보)
SL:     ATR(14) × 2.5  (노이즈에 털리지 않도록 여유 확보)
시간:   30봉 (30분)   (세션 내 추세 확립 에너지 유효 시간)
Friction: 30pt 차감  (IC Markets 스프레드+슬리피지, A2 원칙)
TP 판정: Close 기준 (위크 배제 — 보수적 판정)
```

> [!NOTE]
> 전체 봉에 대해 라벨을 생성합니다. BSPScale은 고정 필터가 아닌 AI 학습 피처로 활용합니다.
> 해당 구간 내 봉에서 AI가 눌림목 여부 / 타이밍 / 진입 여부를 학습합니다.
...
# ── Setup 필터 제거 (2026-03-07): BSPScale은 AI 학습 피처로 활용 ──
df_setup = df  # 전체 봉 사용
